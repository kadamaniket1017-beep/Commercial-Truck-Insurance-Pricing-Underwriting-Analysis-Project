...
_SELF_KNOWLEDGE_TRIGGERS = [
    ...
    "sssia", "sri sathya sai", "team 5", "tesa joby", "khyati",
    "lakshita", "yashaswi", "siva ruben", "aniket", "rohan gupta",
...
MENTOR TEAM:
  • Sri Sathya Sai Institute of Actuaries (SSSIA) Support Group
    LinkedIn: https://www.aiactuaries.org
  • Sathya Sai Mudigonda
...
DEVELOPER TEAM:
  • Tesa Joby           — Underwriting Analytics & Dataset Expert
...
with st.sidebar:
    ...
    if source == "Use sample dataset (demo)":
        ...
        if st.button("🎲 Generate sample dataset", use_container_width=True) or st.session_state.sample_df is None:
    ...
    st.markdown(
        "- [Sri Sathya Sai Institute of Actuaries Support Group](https://www.aiactuaries.org)\n"
    ...
    st.markdown(
        "- [Tesa Joby](https://www.linkedin.com/in/tesa-joby-02b28b243) — Underwriting Analytics & Dataset Expert\n"
...
if not engine.ready:
    st.markdown("""
Welcome to **TruckShield AI** — the full 37-column dataset is now integrated across all agents.

| Stage | Agent | What's in Each Agent Schema|
|---|---|---|
| 1 | **Data Validation** | Validates 17 new columns; uses real claims data for XGBoost targets |
| 2 | **Climate Risk** | Climate zone modifiers + Route Risk Score (route type × highway × destinations) |
| 3 | **Risk Scoring** | Driver Behavior Score replaces Telematics; Night Travel Risk score added |
...
with tabs[0]:
    ...
    if col_search2.button("CHECK", use_container_width=True):
        if search_query:
    ...
    with st.form("quote_form"):
        ...
        yrs_since = cl3.number_input(
            ...
            min_value=0.0, max_value=99.0, value=float(get_def("years_since_last_claim", 0.0)), step=1,
