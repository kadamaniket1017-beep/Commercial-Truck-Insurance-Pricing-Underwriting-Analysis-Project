...
RAG_SIMILARITY_THRESHOLD = 0.10   # below → HIGH RISK flag
RAG_MEDIUM_THRESHOLD     = 0.20   # below → MEDIUM / CAUTION flag
    ...
    def call_gemini(self, prompt: str, max_tokens: int = 350,
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                # The check is lightweight (pure-Python TF-IDF) so it adds
                # negligible latency.  Results are logged in the reasoning
                # chain and surface in the Agent Reasoning Chain tab.
                try:
...
if st.session_state.validation_pending:
    ...
    st.caption(
        "🚨 **Warning-** RAG uses Wikipedia as a source to verify model is hallucinating or not. "
        "Since we don't have clear documentation of data from other website, regulatory report, methods used, "
        "RAG will result with red flag showing 100% hallucination even it is not hallucinating.\n\n"
        "**Reason-** e.g. we are using a data from particular website which may be 2-3 year old & RAG will "
        "look through Wikipedia to verify it, it may ended with latest/ other results causing red flag from RAG[cite: 1]"
    )
...
| 3 — SEM Model Data | TruckShield engine | State risk indices, cargo risk scores, route risk maps |

**Similarity thresholds:**
| Score | Status | Meaning |
|-------|--------|---------|
| ≥ 0.20 | 🟢 VERIFIED | Output aligns with KB |
| 0.10 – 0.20 | 🟡 CAUTION | Low overlap; review key figures |
| < 0.10 | 🔴 FLAGGED | Very low overlap; possible hallucination |
...
""")
        ...
        # ── RAG results from this pipeline run ────────────────────────
