...
#   Each of the 5 agents is a fully autonomous Agent object with:
#     • Its own LLM reasoning step (Gemini)
#     • Typed AgentMessage inter-agent communication protocol
#     • Orchestrator routing with full reasoning chain audit
#
#   FOUR FULL-AGENTIC CAPABILITIES:
#
#   1. AGENTS CAN RETRY OR LOOP  ──────────────────────────────
#      Every agent uses retry_with_backoff() — up to 3 attempts
#      with exponential delay. If Gemini returns an error or a
#      low-confidence response, the agent loops and tries again
#      with an improved prompt. Retries are logged in the chain.
#
#   2. AGENTS SPAWN SUB-AGENTS  ───────────────────────────────
#      RiskScoringAgent dynamically spawns CargoSubAgent and
#      DriverSubAgent to handle specialised sub-scoring in
#      parallel. UnderwritingAgent spawns a FraudScreenSubAgent
#      when anomaly signals are detected. Sub-agents have their
#      own memory and emit messages back to the parent agent.
#
#   3. TOOL USE BY AGENTS  ────────────────────────────────────
#      Every agent now has access to a tool registry and calls
#      tools as part of its own reasoning pipeline (not just in
#      the AI tab). Tools available to agents:
#        • wikipedia_tool   — background knowledge lookup
#        • weather_tool     — live weather & visibility data
#        • accident_stats_tool — NCRB/MoRTH accident data API
#      Tool results are injected into the Gemini reasoning prompt
#      and logged in the Agent Reasoning Chain tab.
#
#   4. RAG — HALLUCINATION GUARD  ─────────────────────────────
#      A Retrieval-Augmented Generation (RAG) layer sits over
#      every Gemini output. It uses a curated knowledge base of
#      verified IRDAI regulations, NCRB statistics, actuarial
#      definitions, and Indian road safety facts to detect when
#      the LLM may have hallucinated. Sources:
#        • Embedded domain KB  — IRDAI rules, SEM calibrations
#        • Wikipedia REST API  — live open-source fact lookup
#        • NCRB/MoRTH dataset  — embedded accident statistics
#      The RAG guard scores each Gemini response for semantic
#      alignment with verified facts. Flagged statements are
#      shown in the "🤖 Agent Reasoning Chain" tab with an
#      explanation of what was retrieved vs. what was stated.
#
#   AgentMessage  — typed envelope: sender, receiver, payload, reasoning
#   AgentMemory   — per-agent log of all messages sent/received
#   Orchestrator  — coordinates agent execution order, delivers messages,
#                   logs the full reasoning chain visible in the UI
#
#   Without a Gemini key: falls back to deterministic pipeline.
#   With a Gemini key:    full agentic mode — reasoning visible in new
#                         "🤖 Agent Reasoning Chain" tab.
# ============================================================
...
# ============================================================
# AgentMessage — the inter-agent communication envelope.
# Every piece of information that flows between agents is
# wrapped in this structure so the Orchestrator can log it,
# display it, and route it to the correct recipient.
# ============================================================
...
# ============================================================
# RAG — RETRIEVAL-AUGMENTED GENERATION HALLUCINATION GUARD
# ============================================================
# PURPOSE
# ───────
# Gemini (and any open-source Wikipedia platform) can hallucinate
# domain-specific figures — wrong TP tariff amounts, wrong IRDAI
# NCB percentages, wrong MoRTH statistics, wrong cargo risk
# classifications, etc.
#
# This RAGKnowledgeBase solves that in three ways:
#   1. Embedded KB  — a curated set of verified facts from IRDAI
#      regulations, NCRB/MoRTH accident reports, and the SEM
#      pricing model calibration.  These are ground-truth strings
#      that cannot hallucinate because we wrote them.
#   2. Wikipedia Retrieval — on-demand live lookup via the
#      Wikipedia REST API (open-source, no key required) to
#      enrich the KB with technical background.
#   3. Hallucination Scoring — a TF-IDF cosine-similarity check
#      compares every Gemini output against the retrieved context.
#      If the similarity drops below a configurable threshold the
#      text is flagged, the retrieved evidence is shown, and the
#      Reasoning Chain tab surfaces a warning to the user.
#
# THRESHOLDS (document-level, adjustable)
# ────────────────────────────────────────
#   RAG_SIMILARITY_THRESHOLD  = 0.10  (0 = no overlap, 1 = identical)
#      Below this → HIGH RISK hallucination flag
#   RAG_MEDIUM_THRESHOLD      = 0.20
#      Below this → MEDIUM RISK caution flag
#   Above RAG_MEDIUM_THRESHOLD → VERIFIED / low risk
#
# The thresholds are intentionally conservative (low) because
# insurance regulatory text uses specialised vocabulary that
# rarely overlaps with general Wikipedia prose.  The goal is to
# catch clear mis-statements (wrong numbers, wrong names) rather
# than penalise correct paraphrases.
#
# HOW TO READ THE OUTPUT (Agent Reasoning Chain tab)
# ───────────────────────────────────────────────────
#   🟢 VERIFIED   — output aligns with retrieved context
#   🟡 CAUTION    — low overlap; may contain imprecise wording
#   🔴 FLAGGED    — very low overlap; review manually
#   Retrieved snippets are shown so the user can judge for
#   themselves what the ground truth says.
# ============================================================

RAG_SIMILARITY_THRESHOLD = 0.05   # below → HIGH RISK flag  (lowered: domain vocab mismatch is expected)
RAG_MEDIUM_THRESHOLD     = 0.12   # below → MEDIUM / CAUTION flag  (lowered to reduce false positives)

# Agents whose output should NOT be RAG-checked.
#
# ValidationAgent — output is about data schema errors (missing fields, wrong types).
#   No KB entry covers "row 42 has a missing GVW field", so comparing against IRDAI
#   tariff documents always scores near-zero → misleading FLAGGED.
#
# Sub-agents (CargoSubAgent, DriverSubAgent, FraudScreenSubAgent) — their Gemini output
#   is internal numerical reasoning: "cargo risk score is 6.2 because hazardous goods..."
#   These are computed judgements about pipeline data, not regulatory statements.
#   Comparing them against IRDAI/NCRB KB snippets always gives near-zero TF-IDF overlap
#   even when the reasoning is correct → misleading FLAGGED.
#
# The 4 main domain agents (ClimateAgent, RiskScoringAgent, UnderwritingAgent,
# PricingAgent) are still RAG-checked — their outputs reference IRDAI tariffs, NCB
# grids, MoRTH statistics, and OD rate bounds that the KB can actually verify.
RAG_EXCLUDED_AGENTS = {
    "ValidationAgent",      # data quality output — no domain KB match
}

# Sub-agents that use Wikipedia tool — RAG-checked; flagged when no user docs.
RAG_SUBAGENT_SET = {
    "CargoSubAgent",
    "DriverSubAgent",
    "FraudScreenSubAgent",
...
# ── Embedded verified knowledge base ─────────────────────────
# Each entry: {"id": str, "topic": str, "text": str, "source": str}
# Sources: IRDAI Motor Insurance Regulations, MoRTH Road Accident
#   Report 2022, NCRB 2022, NDMA State Disaster Plans, SEM calibration.
RAG_EMBEDDED_KB: list[dict] = [
    # ── IRDAI / Regulatory ─────────────────────────────────────
    {
        "id": "irdai_tp_mandatory",
        "topic": "Third Party Insurance",
        "text": (
            "Third Party (TP) motor insurance is compulsory for all vehicles in India "
            "under the Motor Vehicles Act 1988. The TP premium is fixed by IRDAI and "
            "MoRTH based on the Gross Vehicle Weight (GVW) slab. No insurer can offer a "
            "discount on TP premium — it is non-negotiable and the same across all insurers."
        ),
        "source": "IRDAI Motor Tariff / Motor Vehicles Act 1988",
    },
    {
        "id": "irdai_ncb_grid",
        "topic": "No Claim Bonus NCB",
        "text": (
            "Under IRDAI's NCB schedule for commercial vehicles, the No Claim Bonus "
            "applies only to the Own Damage (OD) component, never to Third Party (TP). "
            "The NCB discount grid is: 0 claim-free years = 0%, 1 year = 20%, "
            "2 years = 25%, 3 years = 35%, 4 years = 45%, 5 or more years = 50%. "
            "Any claim in a policy year resets the NCB tier to zero."
        ),
        "source": "IRDAI Motor Insurance NCB Schedule",
    },
    {
        "id": "irdai_od_rate",
        "topic": "Own Damage OD Premium",
        "text": (
            "Own Damage (OD) premium for commercial vehicles in India is insurer-priced "
            "within IRDAI-approved rate bands. The base OD rate is typically 2.5% of the "
            "Insured Declared Value (IDV). The minimum OD rate floor is 1.5% of IDV and "
            "the maximum cap is 9.0% of IDV. The IDV itself is set as the manufacturer's "
            "listed price adjusted for depreciation by vehicle age."
        ),
        "source": "IRDAI Motor Insurance Regulations",
    },
    {
        "id": "irdai_ncb_protect",
        "topic": "NCB Protect Add-on",
        "text": (
            "IRDAI permits NCB Protect as a commercial vehicle add-on. It preserves the "
            "No Claim Bonus after a single minor claim during a policy year. Without NCB "
            "Protect, any claim — however small — resets the NCB to tier zero, losing "
            "up to 50% OD discount accumulated over five claim-free years."
        ),
        "source": "IRDAI Motor Insurance Add-on Products",
    },
    {
        "id": "irdai_deductible",
        "topic": "Voluntary Deductible",
        "text": (
            "A voluntary deductible is the amount the policyholder agrees to pay out of "
            "pocket for each Own Damage claim before the insurer pays. Opting for a higher "
            "voluntary deductible typically reduces the OD gross premium by 5–10%. This is "
            "separate from the compulsory deductible mandated by IRDAI for commercial vehicles."
        ),
        "source": "IRDAI Motor Insurance Regulations",
    },
    {
        "id": "irdai_commercial_vehicle_class",
        "topic": "Vehicle Classification GVW",
        "text": (
            "Indian commercial vehicles are classified by Gross Vehicle Weight (GVW): "
            "Light Motor Vehicle LMV under 3.5 tonnes, Light Commercial Vehicle LCV "
            "3.5 to 7.5 tonnes, Medium Commercial Vehicle MCV 7.5 to 12 tonnes, "
            "Heavy Commercial Vehicle HCV 12 to 25 tonnes, Multi-Axle above 25 tonnes. "
            "The TP tariff slab and many risk parameters are tied to this GVW classification."
        ),
        "source": "Ministry of Road Transport and Highways India / IRDAI",
    },
    {
        "id": "bs_emission_norms",
        "topic": "Bharat Stage Emission Norms BS-VI",
        "text": (
            "India adopted Bharat Stage VI (BS-VI) emission standards for all new vehicles "
            "from April 2020, leapfrogging BS-V. BS-VI trucks have significantly lower "
            "particulate matter and NOx emissions than BS-IV and earlier norms. "
            "For insurance pricing, BS-VI vehicles are classified as lower risk because "
            "they have more advanced engine management systems that reduce mechanical "
            "failure probability compared to BS-II and BS-III vehicles."
        ),
        "source": "Ministry of Road Transport MoRTH Notification 2020",
    },
    # ── NCRB / MoRTH Accident Statistics ──────────────────────
    {
        "id": "morth_night_accidents",
        "topic": "Night Driving Accident Risk",
        "text": (
            "According to MoRTH Road Accident Report 2022, approximately 45% of fatal "
            "road accidents in India occur between 6 PM and 6 AM despite only about 30% "
            "of total traffic being on road at night. Commercial trucks are disproportionately "
            "involved in night-time fatal accidents due to fatigue, poor visibility, and "
            "less emergency response infrastructure on national and state highways at night."
        ),
        "source": "MoRTH Road Accident Report 2022",
    },
    {
        "id": "ncrb_overloading_risk",
        "topic": "Overloading Risk Trucks",
        "text": (
            "NHAI data and NCRB reports indicate that overloaded commercial vehicles have "
            "approximately twice (2x) the brake failure rate of correctly loaded trucks. "
            "Overloading increases tyre blow-out risk, reduces braking effectiveness, "
            "and elevates the severity of accidents. The CMVR (Central Motor Vehicles Rules) "
            "prohibits loading beyond the registered GVW."
        ),
        "source": "NHAI / NCRB / CMVR Regulations",
    },
    {
        "id": "morth_accident_uttar_pradesh",
        "topic": "Road Accidents State Risk India",
        "text": (
            "MoRTH Road Accident Report 2022 identifies Uttar Pradesh, Tamil Nadu, and "
            "Madhya Pradesh as the top three states by total road accident fatalities in India. "
            "National highways account for 31% of road accidents but only 2% of the total "
            "road length, indicating high-risk concentration on inter-city freight corridors. "
            "Hill routes in Uttarakhand and Himachal Pradesh have among the highest per-km "
            "accident rates due to sharp curves and landslide-prone terrain."
        ),
        "source": "MoRTH Road Accident Report 2022",
    },
    {
        "id": "morth_cargo_hazardous",
        "topic": "Hazardous Goods Transport",
        "text": (
            "Transport of hazardous goods in India requires a Hazardous Materials Road "
            "Transport (HMRT) permit and a National Permit under the Motor Vehicles Act. "
            "Carriers of chemical or petroleum products must comply with CMVR Rule 129 "
            "which mandates a trained driver, emergency equipment, and specific labelling. "
            "Insurance for hazardous cargo typically attracts higher OD loadings and "
            "mandatory REFER underwriting review."
        ),
        "source": "CMVR Rule 129 / MoRTH Hazardous Goods Notification",
    },
    {
        "id": "adas_safety",
        "topic": "ADAS Advanced Driver Assistance Systems",
        "text": (
            "Advanced Driver Assistance Systems (ADAS) for commercial trucks include "
            "lane departure warning, forward collision warning, automatic emergency braking, "
            "and driver drowsiness detection. AIS-140 is the Indian standard for vehicle "
            "location tracking devices fitted to commercial vehicles. ADAS adoption in Indian "
            "commercial fleets remains below 15% as of 2023 due to high installation costs, "
            "but NHAI data shows 30–40% accident reduction on routes with ADAS-equipped fleets."
        ),
        "source": "AIS-140 Standard / NHAI Safety Report",
    },
    {
        "id": "climate_flood_india",
        "topic": "Flood Risk India States",
        "text": (
            "India's flood-prone states include Assam (Brahmaputra basin), Bihar (Kosi and "
            "Gandak rivers), West Bengal, Odisha, Uttar Pradesh, and Kerala. The Indo-Gangetic "
            "Plain experiences seasonal monsoon flooding every year. NDMA (National Disaster "
            "Management Authority) classifies flood risk on a state-by-state basis. Assam "
            "typically records the highest annual flood damage to road infrastructure, "
            "followed by Bihar and West Bengal."
        ),
        "source": "NDMA State Disaster Management Plans",
    },
    {
        "id": "climate_cyclone_india",
        "topic": "Cyclone Risk India Coastal States",
        "text": (
            "India's cyclone-prone coastline includes the Bay of Bengal coast — Odisha, "
            "West Bengal, Andhra Pradesh, Tamil Nadu — and the Arabian Sea coast — Gujarat "
            "and Kerala. Odisha and Andhra Pradesh are historically most impacted by severe "
            "cyclones. The Indian Meteorological Department IMD classifies cyclone risk using "
            "a 5-tier intensity scale. Cyclone season is June–November on the east coast "
            "and May–June on the west coast."
        ),
        "source": "IMD Cyclone Atlas / NDMA",
    },
    {
        "id": "climate_fog_india",
        "topic": "Dense Fog Risk North India",
        "text": (
            "Dense fog is a major road safety hazard in the Indo-Gangetic Plain (North India) "
            "from November to February. States most affected: Punjab, Haryana, Delhi NCR, "
            "Uttar Pradesh, and Bihar. Fog-related truck accidents spike significantly during "
            "winter months, with visibility sometimes dropping below 50 metres. The IMD "
            "issues Dense Fog Alerts (DFA) for these states during winter."
        ),
        "source": "IMD Dense Fog Alert / MoRTH Winter Advisory",
    },
    # ── XGBoost / Actuarial methodology ───────────────────────
    {
        "id": "xgboost_poisson",
        "topic": "XGBoost Poisson Frequency Model",
        "text": (
            "XGBoost with objective='count:poisson' is used to model claim frequency "
            "(expected number of claims per policy year). The Poisson distribution is the "
            "canonical actuarial model for count data because claims are rare independent "
            "events over a fixed exposure period. The Poisson log-likelihood is directly "
            "optimised, which is more appropriate than minimising mean squared error for "
            "integer count targets."
        ),
        "source": "Chen & Guestrin (2016) XGBoost / Actuarial Frequency Modelling",
    },
    {
        "id": "xgboost_gamma",
        "topic": "XGBoost Gamma Severity Model",
        "text": (
            "XGBoost with objective='reg:gamma' models claim severity (average cost per claim). "
            "The Gamma distribution is suitable for severity because claim amounts are strictly "
            "positive and right-skewed — many small claims and a few catastrophically large ones. "
            "The Gamma deviance objective avoids the MSE problem of being dominated by outliers. "
            "Pure premium = E[frequency] × E[severity] is the compound Poisson-Gamma framework "
            "standard in non-life actuarial pricing."
        ),
        "source": "Actuarial Compound Poisson-Gamma Framework",
    },
    {
        "id": "commercial_fleet_india",
        "topic": "Commercial Truck Fleet India",
        "text": (
            "India has approximately 10 million registered commercial vehicles. "
            "The commercial vehicle sector accounts for about 70% of freight movement "
            "by road. The average fleet owner operates 2–3 vehicles. The organised fleet "
            "sector (5+ vehicles) constitutes about 15% of operators but carries 40% of "
            "freight volume. MoRTH data indicates a claim frequency of approximately 4–5% "
            "for commercial vehicles annually — meaning 4–5 out of every 100 insured "
            "vehicles file at least one OD claim per year."
        ),
        "source": "MoRTH Annual Report / IRDAI Annual Report",
    },
    {
        "id": "cng_truck_safety",
        "topic": "CNG LNG Fuel Safety Trucks",
        "text": (
            "Compressed Natural Gas (CNG) and Liquefied Natural Gas (LNG) are increasingly "
            "used in Indian commercial trucks as alternatives to diesel. CNG trucks have "
            "lower fire risk per km than diesel because CNG disperses upward (lower density "
            "than air) while diesel pools on the ground. However, high-pressure CNG cylinders "
            "require periodic hydrostatic testing. LNG trucks carry cryogenic fuel which "
            "requires specialised handling. Both CNG and LNG are considered lower fire-risk "
            "than diesel by most Indian commercial vehicle insurers."
        ),
        "source": "PESO / IRDAI Technical Circular on Alternative Fuels",
    },
    {
        "id": "driver_training_aimtc",
        "topic": "Driver Training India Commercial Vehicles",
        "text": (
            "The All India Motor Transport Congress (AIMTC) and state Road Transport "
            "Corporations operate formal driver training programmes for commercial vehicle "
            "drivers in India. Government-accredited training centres issue skill certificates "
            "under the PMKVY (Pradhan Mantri Kaushal Vikas Yojana) scheme. Trained drivers "
            "show 20–30% lower at-fault accident rates than untrained peers according to "
            "NCRB data. IRDAI encourages training incentives in commercial motor insurance."
        ),
        "source": "AIMTC / PMKVY / NCRB",
    },
    {
        "id": "gps_ais140_telematics",
        "topic": "AIS-140 GPS Telematics Commercial Vehicles",
        "text": (
            "AIS-140 is the mandatory vehicle location tracking device standard for commercial "
            "vehicles in India, notified by MoRTH. All new commercial vehicles above 7.5T GVW "
            "must be fitted with an AIS-140 compliant GPS device. Major Indian insurers "
            "including New India Assurance, ICICI Lombard, and Bajaj Allianz offer Usage-Based "
            "Insurance (UBI) discounts of 5–10% on OD premium for policyholders who share "
            "verified telematics data showing safe driving behaviour."
        ),
        "source": "MoRTH AIS-140 Notification / IRDAI UBI Guidelines",
    },
]

# ── Wikipedia topics to pre-fetch for RAG enrichment ─────────
# These are fetched once on first use and cached in session state
RAG_WIKIPEDIA_TOPICS = [
    ("Insurance Regulatory and Development Authority India", "irdai_wiki"),
    ("Ministry of Road Transport and Highways India",        "morth_wiki"),
    ("Bharat Stage emission standards",                      "bs_norms_wiki"),
    ("XGBoost gradient boosting",                           "xgboost_wiki"),
    ("Advanced driver-assistance systems",                  "adas_wiki"),
    ("Poisson distribution",                                "poisson_wiki"),
]


class RAGKnowledgeBase:
    """
    Retrieval-Augmented Generation knowledge base for TruckShield AI.

    Maintains a searchable index of verified domain facts (embedded KB +
    optionally enriched with Wikipedia).  Exposes two public methods:

      retrieve(query, top_k)  → list of the top-k most relevant facts
      check_hallucination(text, context_docs) → dict with flag + evidence

    The similarity metric is TF-IDF cosine similarity computed with only
    the Python standard library (no scikit-learn dependency added here)
    so the RAG layer adds zero new package requirements.
    """

    def __init__(self):
        self._docs: list[dict] = list(RAG_EMBEDDED_KB)   # mutable copy
        self._tfidf_index: list[dict] = []   # {id, terms: Counter, norm}
        self._build_index()
        self.fetch_log: list[str] = []       # log of Wikipedia fetches
        self._user_docs_added: int = 0       # count of user-supplied custom documents

    def add_user_document(self, text: str, source_label: str = "User Document"):
        """
        Add a plain-text document supplied by the user (e.g. from a Google Drive
        link or uploaded file).  These are added to the KB with higher weight so
        they anchor the hallucination check against the *actual* model sources
        rather than generic Wikipedia articles.
        """
        if not hasattr(self, "_user_docs_added"):
            self._user_docs_added = 0
        self._user_docs_added += 1
        doc_id = f"user_doc_{self._user_docs_added}"
        self.add_document(
            doc_id=doc_id,
            topic=f"User-supplied reference ({self._user_docs_added})",
            text=text[:4000],   # cap at 4 KB per doc for TF-IDF performance
            source=source_label,
        )
        self.fetch_log.append(f"✅ User document added: '{source_label}' ({len(text)} chars)")

    def fetch_google_drive_text(self, drive_url: str) -> bool:
        """
        Attempt to fetch plain text from a publicly shared Google Drive document.
        Supports both /view and /export URLs.  Returns True if content was added.
        """
        try:
            # Convert /view URL → /export?format=txt for Drive Docs
            if "docs.google.com/document" in drive_url:
                file_id = re.search(r"/d/([a-zA-Z0-9_-]+)", drive_url)
                if file_id:
                    export_url = f"https://docs.google.com/document/d/{file_id.group(1)}/export?format=txt"
                else:
                    export_url = drive_url
            elif "drive.google.com/file" in drive_url:
                file_id = re.search(r"/d/([a-zA-Z0-9_-]+)", drive_url)
                if file_id:
                    export_url = f"https://drive.google.com/uc?export=download&id={file_id.group(1)}"
                else:
                    export_url = drive_url
            else:
                export_url = drive_url
            ...
            with urllib.request.urlopen(req, timeout=10) as resp:
                raw = resp.read()
            # Try decode as UTF-8 then latin-1
            try:
            ...
            text = text.strip()
            if len(text) < 50:
                self.fetch_log.append(f"⚠️ Drive fetch returned too little text from: {drive_url[:60]}")
                return False
            self.add_user_document(text, source_label=f"Google Drive: {drive_url[:60]}")
            return True
        except Exception as exc:
            self.fetch_log.append(f"⚠️ Google Drive fetch failed ({drive_url[:60]}): {str(exc)[:80]}")
            return False

    # ── Index construction ────────────────────────────────────
    def _tokenise(self, text: str) -> list[str]:
        """Lower-case, split on non-alpha, remove single-char tokens."""
        tokens = re.findall(r"[a-z]{2,}", text.lower())
        # Simple stop-word removal
        stops = {
            "the", "a", "an", "and", "or", "in", "of", "to", "for", "is",
            "are", "it", "its", "as", "be", "by", "on", "at", "with",
            "this", "that", "these", "those", "has", "have", "had",
            "from", "into", "than", "which", "such", "also", "both",
            "all", "each", "per", "can", "was", "were", "not", "more",
        }
        return [t for t in tokens if t not in stops]

    def _tf(self, tokens: list[str]) -> "collections.Counter":
        return collections.Counter(tokens)

    def _build_index(self):
        """Build TF-IDF inverted index over all documents."""
        # Step 1: term frequencies per document
        doc_tfs: list[tuple[dict, collections.Counter]] = []
        for doc in self._docs:
            combined = doc["topic"] + " " + doc["text"]
            tokens   = self._tokenise(combined)
            tf       = self._tf(tokens)
            doc_tfs.append((doc, tf))

        # Step 2: document frequencies for IDF
        df_counts: collections.Counter = collections.Counter()
        for _, tf in doc_tfs:
            for term in tf:
                df_counts[term] += 1
        n_docs = max(len(self._docs), 1)

        # Step 3: TF-IDF vectors + L2 norms
        self._tfidf_index = []
        for doc, tf in doc_tfs:
            tfidf_vec: dict[str, float] = {}
            for term, freq in tf.items():
                idf = _math.log((n_docs + 1) / (df_counts[term] + 1)) + 1.0
                tfidf_vec[term] = freq * idf
            norm = _math.sqrt(sum(v * v for v in tfidf_vec.values())) or 1.0
            self._tfidf_index.append({
                "id":   doc["id"],
                "doc":  doc,
                "vec":  tfidf_vec,
                "norm": norm,
    ...
    def _cosine(self, query_vec: dict[str, float], query_norm: float,
                doc_entry: dict) -> float:
        """Cosine similarity between a query TF-IDF vector and an indexed doc."""
        if query_norm == 0 or doc_entry["norm"] == 0:
            return 0.0
        dot = sum(query_vec.get(t, 0) * v for t, v in doc_entry["vec"].items())
        return dot / (query_norm * doc_entry["norm"])

    # ── Public API ────────────────────────────────────────────
    def add_document(self, doc_id: str, topic: str, text: str, source: str):
        """Add a new document (e.g. from Wikipedia) and rebuild index."""
        self._docs.append({"id": doc_id, "topic": topic, "text": text, "source": source})
        self._build_index()

    def has_user_docs(self) -> bool:
        """Return True if the user has uploaded at least one custom document."""
        return getattr(self, "_user_docs_added", 0) > 0

    def retrieve(self, query: str, top_k: int = 3) -> list[dict]:
        """Return top_k most semantically relevant documents for the query."""
        q_tokens = self._tokenise(query)
        q_tf     = self._tf(q_tokens)
        n_docs   = max(len(self._docs), 1)

        # IDF uses the same corpus frequencies as the index
        df_counts: collections.Counter = collections.Counter()
        for entry in self._tfidf_index:
            for term in entry["vec"]:
                df_counts[term] += 1

        q_vec: dict[str, float] = {}
        for term, freq in q_tf.items():
            idf = _math.log((n_docs + 1) / (df_counts.get(term, 0) + 1)) + 1.0
            q_vec[term] = freq * idf
        q_norm = _math.sqrt(sum(v * v for v in q_vec.values())) or 1.0

        # User docs get a 2x score boost so they surface first when uploaded.
        user_docs_present = self.has_user_docs()
        scored = []
        for entry in self._tfidf_index:
            score = self._cosine(q_vec, q_norm, entry)
            if user_docs_present and entry["doc"].get("id", "").startswith("user_doc_"):
                score = min(score * 2.0, 1.0)   # boost user docs; cap at 1.0
            scored.append((score, entry["doc"]))
        scored.sort(key=lambda x: x[0], reverse=True)
        return [doc for _, doc in scored[:top_k]]

    def check_hallucination(self, gemini_text: str,
                            context_docs: list[dict] | None = None,
                            query: str = "",
                            agent_name: str = "") -> dict:
        """
        Compare a Gemini output against the knowledge base.

        Returns a dict:
          status          : "VERIFIED" | "CAUTION" | "FLAGGED"
          similarity      : float (0–1)
          retrieved_docs  : list of the top-3 most relevant KB entries
          flag_reason     : human-readable explanation
          evidence        : list of short source snippets shown to the user
        """
        if context_docs is None:
            context_docs = self.retrieve(query or gemini_text, top_k=3)
        ...
        if not context_docs:
            return {
                ...
                "similarity": 0.0,
                "retrieved_docs": [],
                "flag_reason": "No relevant KB documents found to verify against.",
                "evidence": [],
            }

        # Build a combined context string and compute similarity
        combined_context = " ".join(d["text"] for d in context_docs)
        q_tokens  = self._tokenise(gemini_text)
        ctx_tokens = self._tokenise(combined_context)
        ...
        if not q_tokens or not ctx_tokens:
            return {
                ...
                "flag_reason": "Could not tokenise text for similarity check.",
                "evidence": [d["text"][:200] for d in context_docs],
            }

        # Jaccard similarity as a secondary measure (robust for short texts)
        q_set   = set(q_tokens)
        ctx_set = set(ctx_tokens)
        jaccard  = len(q_set & ctx_set) / max(len(q_set | ctx_set), 1)

        # TF cosine on the combined context
        q_tf  = self._tf(q_tokens)
        ctx_tf = self._tf(ctx_tokens)
        all_terms = set(q_tf) | set(ctx_tf)
        n = max(len(all_terms), 1)
        q_arr   = [q_tf.get(t, 0) for t in all_terms]
        ctx_arr = [ctx_tf.get(t, 0) for t in all_terms]
        dot = sum(a * b for a, b in zip(q_arr, ctx_arr))
        norm_q   = _math.sqrt(sum(a * a for a in q_arr)) or 1.0
        norm_ctx = _math.sqrt(sum(b * b for b in ctx_arr)) or 1.0
        cosine   = dot / (norm_q * norm_ctx)

        # Combined score (weighted average)
        similarity = round(0.6 * cosine + 0.4 * jaccard, 4)

        # -- Data source detection -----------------------------------------
        has_user = self.has_user_docs()
        any_user_doc = any(d.get("id", "").startswith("user_doc_") for d in context_docs)
        any_wiki     = any("Wikipedia" in d.get("source", "") for d in context_docs)
        is_subagent  = agent_name in RAG_SUBAGENT_SET

        if any_user_doc:
            data_source  = "user_docs"
            source_label = "📎 User-uploaded documentation"
        elif any_wiki:
            data_source  = "wikipedia"
            source_label = "🌐 Wikipedia (no user docs uploaded)"
        else:
            data_source  = "embedded_kb"
            source_label = "🗒️ Built-in IRDAI/NCRB knowledge base"

        # -- Build contextual flag messages --------------------------------
        if similarity < RAG_SIMILARITY_THRESHOLD:
            status = "FLAGGED"
            if is_subagent and not has_user:
                flag_reason = (
                    f"🚨 HALLUCINATION RISK — {agent_name} sourced context from Wikipedia "
                    f"because no user documentation was uploaded. "
                    f"Similarity {similarity:.3f} < {RAG_SIMILARITY_THRESHOLD} vs verified IRDAI/NCRB KB. "
                    f"Upload your model documentation via '📂 Add Custom RAG Documents' "
                    f"to anchor this agent's reasoning against verified sources."
                )
            else:
                flag_reason = (
                    f"Similarity {similarity:.3f} < high-risk threshold ({RAG_SIMILARITY_THRESHOLD}). "
                    f"LLM output has low overlap with verified knowledge. "
                    f"Data source: {source_label}. Review manually."
                )
        elif similarity < RAG_MEDIUM_THRESHOLD:
            status = "CAUTION"
            if is_subagent and not has_user:
                flag_reason = (
                    f"⚠️ CAUTION — {agent_name} is reasoning from Wikipedia context "
                    f"(no user docs uploaded). Similarity {similarity:.3f} < "
                    f"medium-risk threshold ({RAG_MEDIUM_THRESHOLD}). "
                    f"Upload your model documentation to improve grounding."
                )
            else:
                flag_reason = (
                    f"Similarity {similarity:.3f} < medium-risk threshold ({RAG_MEDIUM_THRESHOLD}). "
                    f"LLM output may contain imprecise wording. "
                    f"Data source: {source_label}. Cross-check key figures."
                )
        else:
            status = "VERIFIED"
            flag_reason = (
                f"Similarity {similarity:.3f} >= {RAG_MEDIUM_THRESHOLD}. "
                f"Output aligns with retrieved knowledge base. "
                f"Source: {source_label}."
            )

        evidence = []
        for d in context_docs:
            snippet = f"[{d['source']}] {d['text'][:200]}…"
    ...
    def fetch_wikipedia_and_add(self, query: str, doc_id: str,
                                 sentences: int = 5) -> bool:
        """
        Fetch a Wikipedia summary and add it to the KB.
        ...
        try:
            ...
            self.add_document(
                doc_id=doc_id,
                topic=page.get("title", title),
                text=summary,
                source=f"Wikipedia — {page.get('content_urls',{}).get('desktop',{}).get('page','')}"
            )
            self.fetch_log.append(f"✅ Added Wikipedia article: '{title}' as '{doc_id}'")
            return True
        except Exception as exc:
            self.fetch_log.append(f"⚠️ Wikipedia fetch failed for '{query}': {str(exc)[:80]}")
            return False

    def bootstrap_wikipedia(self) -> int:
        """
        Pre-fetch a curated set of Wikipedia articles to enrich the KB.
        Called once during Streamlit session initialisation.
        Returns the number of articles successfully added.
        Note: Wikipedia may be unreachable in some environments (e.g. Colab/Cloudflare
        tunnels with restricted egress). Failures are logged but do NOT block the app —
        the embedded IRDAI/NCRB KB is always available as a fallback.
        """
        added = 0
        failed = 0
        for query, doc_id in RAG_WIKIPEDIA_TOPICS:
            if any(d["id"] == doc_id for d in self._docs):
                continue   # already in KB
            if self.fetch_wikipedia_and_add(query, doc_id, sentences=5):
                added += 1
            else:
                failed += 1
        if failed > 0 and added == 0:
            self.fetch_log.append(
                f"⚠️ All {failed} Wikipedia fetches failed. "
                f"This is normal in restricted network environments (Colab/Cloudflare tunnels). "
                f"RAG will use the {len(RAG_EMBEDDED_KB)} embedded IRDAI/NCRB documents instead. "
                f"Upload your own documentation via '📂 Add Custom RAG Documents' to improve coverage."
            )
        elif failed > 0:
            self.fetch_log.append(
                f"ℹ️ {added} Wikipedia articles added, {failed} fetches failed (network restriction)."
            )
        return added


# ── Session-level singleton ───────────────────────────────────
# The RAGKnowledgeBase is created once per Streamlit session so the
# Wikipedia fetch only happens on first load, not on every rerun.
@st.cache_resource(show_spinner=False)
def _get_rag_kb() -> RAGKnowledgeBase:
    kb = RAGKnowledgeBase()
    kb.bootstrap_wikipedia()
    return kb

RAG_KB: RAGKnowledgeBase = _get_rag_kb()
...
class AgentMessage:
    """Typed communication envelope passed between agents."""
    sender:    str          # name of the sending agent
    receiver:  str          # name of the receiving agent
    msg_type:  str          # e.g. "validation_complete", "climate_scores", "risk_scores"
    payload:   Any          # the actual data (dict, DataFrame, etc.)
    reasoning: str = ""     # Gemini's natural-language reasoning for this step
    timestamp: str = ""     # wall-clock time when the message was created

    def __post_init__(self):
        if not self.timestamp:
            _ist = datetime.timezone(datetime.timedelta(hours=5, minutes=30))
            self.timestamp = datetime.datetime.now(_ist).strftime("%H:%M:%S IST")


class AgentMemory:
    """Per-agent store of every message sent and received."""
    ...
    def last_received(self) -> Optional[AgentMessage]:
        return self.inbox[-1] if self.inbox else None

    def summary(self) -> str:
        lines = [f"[{self.agent_name}] Memory — {len(self.inbox)} received, {len(self.outbox)} sent"]
        for m in self.inbox:
            lines.append(f"  ← from {m.sender}: {m.msg_type}")
        for m in self.outbox:
            lines.append(f"  → to   {m.receiver}: {m.msg_type}")
        return "\n".join(lines)
...
class Orchestrator:
    ...
    - Maintains a global message log (audit trail).
    - Routes AgentMessages between agents.
    - Calls each agent's reason() method (Gemini) before run().
    - Exposes the full reasoning chain for display in the UI.
    - retry_with_backoff()  — retries Gemini calls up to max_retries times
                              with exponential backoff; logs each retry attempt.
    - tool_registry         — dict of {tool_name: callable} available to agents.
    - call_tool()           — agents invoke named tools and get structured results.
    - register_sub_agent()  — parent agents can register dynamically spawned
                              sub-agents so the Orchestrator tracks their messages.
    """
    def __init__(self):
        self.message_log:     list[AgentMessage] = []
        self.reasoning_chain: list[dict]         = []   # [{agent, step, reasoning, timestamp}]
        self._gemini_key:     str                = ""

        # ── LLM model selection (V15) ─────────────────────────────────
        # Supported model strings (Google GenAI SDK names):
        #   "gemini-2.5-flash"        → Google Gemini 2.5 Flash          (default, stable GA)
        #   "gemini-3.1-flash-lite"   → Google Gemini 3.1 Flash Lite     (fast, low-cost)
        #   "gemini-3.1-pro-preview"  → Google Gemini 3.1 Pro Preview    (most capable 3.x)
        #   "gemini-3.5-flash"        → Google Gemini 3.5 Flash          (latest GA, frontier)
        self._llm_model:      str                = "gemini-2.5-flash"   # default

        #Tool registry — agents call tools during reasoning ──
        self.tool_registry: dict = {}
        self._register_default_tools()

        #Sub-agent registry — tracks dynamically spawned agents ──
        self.sub_agents: dict = {}   # {sub_agent_name: BaseAgent}
        self._fraud_screen_result: dict = {}   # set by FraudScreenSubAgent, read by _run_underwriting_core
        self._cargo_sub_result:  dict = {}   # set by CargoSubAgent, read by reasoning chain UI
        self._driver_sub_result: dict = {}   # set by DriverSubAgent, read by reasoning chain UI

    # ── LLM model name helpers ───────────────────────────────────────
    ...
    def set_llm_model(self, model_string: str):
        """Set the active LLM model. Accepts the full display name or the API string."""
        # Accept either display name or API string
        reverse = {v: k for k, v in self._MODEL_DISPLAY_NAMES.items()}
        if model_string in reverse:
            self._llm_model = reverse[model_string]
        elif model_string in self._MODEL_DISPLAY_NAMES:
            self._llm_model = model_string
        # else ignore unknown values

    def get_llm_display_name(self) -> str:
        return self._MODEL_DISPLAY_NAMES.get(self._llm_model, self._llm_model)

    def set_gemini_key(self, key: str):
        self._gemini_key = key

    def deliver(self, msg: AgentMessage, target_memory: AgentMemory):
        """Route a message and record it in the global log."""
        target_memory.receive(msg)
        self.message_log.append(msg)

    def log_reasoning(self, agent_name: str, step: str, reasoning: str):
        _ist = datetime.timezone(datetime.timedelta(hours=5, minutes=30))
        self.reasoning_chain.append({
            "agent":     agent_name,
            "step":      step,
            "reasoning": reasoning,
            "time":      datetime.datetime.now(_ist).strftime("%H:%M:%S IST"),
        })

    #FEATURE 1: RETRY WITH BACKOFF ────────────────────────────
    def call_gemini(self, prompt: str, max_tokens: int = 350,
                    max_retries: int = 3, agent_name: str = "Agent") -> str:
        """
        Single-shot Gemini call with automatic retry on failure.
        Uses self._llm_model (set by user choice in sidebar).
        Retries up to max_retries times with exponential backoff (1s, 2s, 4s).
        Each retry attempt is logged in the reasoning chain.
        """
        if not self._gemini_key:
            return "⚡ Deterministic mode (no Gemini key) — formula output only."
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                client   = genai_new.Client(api_key=self._gemini_key)
                response = client.models.generate_content(
                    model=self._llm_model,   # ← uses selected model
                    contents=prompt,
                    config=genai_types.GenerateContentConfig(max_output_tokens=max_tokens),
                )
                result = response.text.strip()

                # ── Loop check: if response is suspiciously short (<20 chars)
                # or contains error signals, retry with an improved prompt ──
                if len(result) < 20 or result.startswith("I cannot") or result.startswith("Error"):
                    raise ValueError(f"Low-quality response on attempt {attempt}: {result[:60]}")

                if attempt > 1:
                    self.log_reasoning(
                        agent_name,
                        f"✅ Retry succeeded (attempt {attempt}/{max_retries})",
                        f"{self.get_llm_display_name()} responded successfully after {attempt} attempts."
                    )

                # ── RAG HALLUCINATION GUARD ────────────────────────────
                # Every Gemini response is checked against the verified KB,
                # EXCEPT agents in RAG_EXCLUDED_AGENTS (e.g. ValidationAgent)
                # whose outputs are about data quality (schema errors, missing
                # values) and have no domain KB entries to compare against —
                # comparing them against IRDAI tariff docs always gives near-zero
                # similarity and produces misleading FLAGGED results.
                if agent_name not in RAG_EXCLUDED_AGENTS:
                  try:
                    rag_check = RAG_KB.check_hallucination(
                        gemini_text=result, query=prompt[:300],
                        agent_name=agent_name
                    )
                    status_icon = {
                        "VERIFIED": "🟢", "CAUTION": "🟡", "FLAGGED": "🔴"
                    }.get(rag_check["status"], "⚪")
                    evidence_lines = "\n".join(
                        f"  • {e[:180]}" for e in rag_check["evidence"][:2]
                    )
                    rag_log = (
                        f"{status_icon} RAG {rag_check['status']} "
                        f"(similarity={rag_check['similarity']:.3f})\n"
                        f"{rag_check['flag_reason']}\n"
                        f"Retrieved evidence:\n{evidence_lines if evidence_lines else '  (none)'}"
                    )
                    self.log_reasoning(agent_name, f"🔍 RAG Hallucination Check", rag_log)
                    # Store the last RAG result so the UI can highlight it
                    if not hasattr(self, "_rag_results"):
                        self._rag_results = []
                    self._rag_results.append({
                        "agent":         agent_name,
                        "status":        rag_check["status"],
                        "similarity":    rag_check["similarity"],
                        "flag_reason":   rag_check["flag_reason"],
                        "evidence":      rag_check["evidence"],
                        "gemini_snippet": result[:300],
                        "data_source":   rag_check.get("data_source", "embedded_kb"),
                        "source_label":  rag_check.get("source_label", "Built-in KB"),
                        "is_subagent":   agent_name in RAG_SUBAGENT_SET,
                    })
                  except Exception as _rag_exc:
                    # RAG check must never block the pipeline, but log the error
                    # so it's visible in the reasoning chain for debugging.
                    _rag_err_msg = str(_rag_exc)[:200]
                    self.log_reasoning(
                        agent_name, "⚠️ RAG Check Error",
                        f"RAG hallucination check failed for {agent_name}: {_rag_err_msg}. "
                        f"Pipeline continues normally — this is a non-blocking diagnostic error."
                    )
                else:
                    # Only ValidationAgent is excluded from RAG checking.
                    self.log_reasoning(
                        agent_name,
                        "🔍 RAG Hallucination Check",
                        f"⬜ RAG check skipped for {agent_name} — "
                        "ValidationAgent output describes data quality issues "
                        "(missing fields, type errors) which have no matching domain KB entries. "
                        "All other agents (including sub-agents) are RAG-checked."
                    )
                # ── END RAG GUARD ──────────────────────────────────────
                ...
                if attempt < max_retries:
                    ...
                    self.log_reasoning(
                        agent_name,
                        f"🔄 Retry {attempt}/{max_retries} — backing off {wait}s",
                        f"{self.get_llm_display_name()} call failed: {last_error}. Retrying in {wait}s."
                    )
                    time.sleep(wait)
                else:
                    self.log_reasoning(
                        agent_name,
                        f"⚠️ All {max_retries} retries exhausted",
                        f"Final error: {last_error}"
                    )

        return f"⚠️ {self.get_llm_display_name()} reasoning unavailable after {max_retries} attempts: {last_error}"

    #FEATURE 3: TOOL USE BY AGENTS ────────────────────────────
    def _register_default_tools(self):
        """Register the default tool set available to all agents."""
        self.tool_registry = {
            "wikipedia_tool":      self._tool_wikipedia,
            "weather_tool":        self._tool_weather,
            "accident_stats_tool": self._tool_accident_stats,
        }

    def call_tool(self, tool_name: str, **kwargs) -> dict:
        """
        Agents call this to invoke a named tool.
        Returns a dict with keys: 'success', 'data', 'error'.
        The Orchestrator logs every tool call in the reasoning chain.
        """
        ...
        try:
            ...
            return {"success": False, "data": {}, "error": str(exc)[:120]}

    def _tool_wikipedia(self, query: str, sentences: int = 4) -> dict:
        """Wikipedia REST API tool — returns summary for a technical concept."""
        ...
        try:
            ...
            result["summary"] = " ".join(sentences_list[:sentences])
    ...
    def _tool_weather(self, location: str) -> dict:
        """Open-Meteo geocoding + weather tool — returns current conditions."""
        result = {"location": location, "temperature_c": None,
                  "wind_kph": None, "visibility_km": None, "description": ""}
        try:
            geo_url = (
                f"https://geocoding-api.open-meteo.com/v1/search?name="
                f"{urllib.parse.quote(location)}&count=1&language=en&format=json"
            )
            ...
            wx_url = (
                ...
                f"&current=temperature_2m,wind_speed_10m,visibility&wind_speed_unit=kmh"
            )
            with urllib.request.urlopen(wx_url, timeout=6) as resp:
                wx_data = json.loads(resp.read().decode())
            cur = wx_data.get("current", {})
            result["temperature_c"]  = cur.get("temperature_2m")
            result["wind_kph"]       = cur.get("wind_speed_10m")
            vis_m = cur.get("visibility")
            result["visibility_km"]  = round(vis_m / 1000, 1) if vis_m is not None else None
            if result["visibility_km"] is not None and result["visibility_km"] < 1.0:
                result["description"] = "⚠️ Very low visibility (fog/haze risk)"
            elif result["wind_kph"] is not None and result["wind_kph"] > 60:
                result["description"] = "⚠️ High wind speed (cyclone/storm risk)"
            else:
                result["description"] = "Normal conditions"
        except Exception:
            pass
        return result

    def _tool_accident_stats(self, state: str = "", route_type: str = "") -> dict:
        """
        Returns embedded MoRTH/NCRB-calibrated accident statistics
        for the given state and route type. Data is sourced from the
        SEM model configuration (already calibrated to NCRB 2022 data).
        """
        from_sem_state  = SEM["accident_prob_state"].get(state, SEM["accident_prob_state"]["DEFAULT"])
        from_sem_route  = SEM["accident_prob_route"].get(route_type, 5.5)
        combined        = round((from_sem_state * 0.5 + from_sem_route * 0.5), 2)
        return {
            "state":            state,
            "route_type":       route_type,
            "state_risk_index": from_sem_state,
            "route_risk_index": from_sem_route,
            "combined_index":   combined,
            "data_source":      "MoRTH/NCRB Road Accident Report 2022 (model-embedded)",
        }

    #FEATURE 2: SUB-AGENT SPAWNING ────────────────────────────
    def register_sub_agent(self, sub_agent: "BaseAgent"):
        """
        Parent agents call this to register a dynamically spawned sub-agent.
        The Orchestrator then tracks its messages and logs its reasoning.
        """
        self.sub_agents[sub_agent.name] = sub_agent
        self.log_reasoning(
            sub_agent.name,
            "🐣 Sub-agent spawned",
            f"{sub_agent.name} was dynamically spawned by its parent agent."
        )
    ...
    def reset(self):
        ...
        self._fraud_screen_result = {}   # reset fraud result between pipeline runs
        self._cargo_sub_result  = {}   # reset cargo sub result between pipeline runs
        self._driver_sub_result = {}   # reset driver sub result between pipeline runs
        self._rag_results       = []   # reset RAG check log between pipeline runs


# ──────────────────────────────────────────────────────────
# BASE AGENT — all five agents inherit from this.
# Each concrete agent holds:
#   - a reference to the shared Orchestrator
#   - its own AgentMemory
#   - a reason() method that calls Gemini (with retry)
#   - a run()    method that does the actual computation
#   - use_tool()     — call any tool from the Orchestrator registry
#   - spawn_sub()    — dynamically create and register a sub-agent
#   - _retry_reason() — loop until a confident Gemini response or max retries
# ──────────────────────────────────────────────────────────
class BaseAgent:
    ...
    def _emit(self, receiver: str, msg_type: str, payload: Any, reasoning: str = "") -> AgentMessage:
        """Create a message, record in own outbox, and return it."""
        msg = AgentMessage(
            sender=self.name, receiver=receiver,
            msg_type=msg_type, payload=payload, reasoning=reasoning
        )
        self.memory.send(msg)
        return msg

    #FEATURE 3: TOOL USE ──────────────────────────────────────
    def use_tool(self, tool_name: str, log_step: str = "", **kwargs) -> dict:
        """
        Call a named tool from the Orchestrator's registry.
        The result is automatically logged in the reasoning chain.
        Returns the tool result dict (keys: success, data, error).
        """
        result = self.orchestrator.call_tool(tool_name, **kwargs)
        label  = log_step or f"🔧 Tool: {tool_name}"
        if result["success"]:
            summary = json.dumps(result["data"], ensure_ascii=False)[:200]
            self.orchestrator.log_reasoning(self.name, label, f"Tool returned: {summary}")
        else:
            self.orchestrator.log_reasoning(self.name, label,
                                            f"Tool failed: {result['error']}")
        return result

    #FEATURE 2: SPAWN SUB-AGENTS ─────────────────────────────
    def spawn_sub(self, sub_class: type, *args, **kwargs) -> "BaseAgent":
        """
        Dynamically spawn a sub-agent of the given class.
        The Orchestrator is automatically passed in and the sub-agent
        is registered so its messages appear in the reasoning chain.
        """
        sub = sub_class(self.orchestrator, *args, **kwargs)
        self.orchestrator.register_sub_agent(sub)
        return sub

    def reason(self, context: dict) -> str:
        """Override in each subclass to call Gemini with agent-specific context."""
        raise NotImplementedError

    def run(self, *args, **kwargs):
        """Override in each subclass to do the actual computation."""
        raise NotImplementedError
...
class NumberedCanvas(canvas.Canvas):
    ...
    def draw_watermark_and_footer(self, page_count):
        ...
        left_lines = [
            "Model- Commercial Truck Insurance Pricing & Underwriting Analysis Multi-Agent Advance (Pro) Model.",
...
def build_chat_history_pdf(chat_history):
    ...
    USABLE_H = PAGE_H - 36 - 72  # top + bottom margins = 684 pt
    MAX_BLOCK_H = USABLE_H - 40  # leave a small safety margin
    ...
    def _chunk_text(text: str, max_chars: int = 2000) -> list:
        """Split very long text into chunks that will fit on one page."""
    ...
    if not chat_history:
        ...
        for msg in chat_history:
            ...
            if role == "user":
                label_para  = Paragraph("👤 USER", user_label_style)
                bg_color    = colors.HexColor("#E3F2FD")
                border_color = colors.HexColor("#90CAF9")
            else:
                label_para  = Paragraph("🤖 TRUCKSHIELD ASSISTANT", asst_label_style)
                bg_color    = colors.HexColor("#E8F5E9")
                border_color = colors.HexColor("#A5D6A7")

            # Split very long responses into ≤2000-char chunks to avoid LayoutError
            text_chunks = _chunk_text(content, max_chars=2000)

            for ci, chunk in enumerate(text_chunks):
                # First chunk gets the label; continuation chunks get a "…continued" note
                if ci == 0:
                    cell_content = [label_para, Paragraph(chunk, msg_style)]
                else:
                    cont_label = ParagraphStyle('ContLabel', fontName="Helvetica-Oblique",
                                               fontSize=9, textColor=colors.grey, spaceAfter=2)
                    cell_content = [
                        Paragraph("(continued)", cont_label),
                        Paragraph(chunk, msg_style),
                    ]

                msg_table = Table([[cell_content]], colWidths=[COL_W])
                ...
                # Only use KeepTogether for short blocks (< 400 pt estimated height)
                # Long blocks are added directly so ReportLab can page-break inside them
                estimated_h = len(chunk) / 80 * 14  # rough: chars / line_width * leading
                if estimated_h < 400:
...
# ============================================================
# SUB-AGENTS (dynamically spawned by parent agents)
# ============================================================
# These are specialised micro-agents created on-demand.
# They have their own AgentMemory and Gemini reasoning step,
# but share the parent's Orchestrator so messages are logged.
# ============================================================

class CargoSubAgent(BaseAgent):
    """
    Sub-agent spawned by RiskScoringAgent.
    Specialises in cargo hazard classification and gives a
    focused Gemini analysis of the specific cargo type being
    priced, pulling Wikipedia context about the commodity.
    """
    def __init__(self, orchestrator: Orchestrator):
        super().__init__("CargoSubAgent", orchestrator)

    def reason(self, context: dict) -> str:
        cargo    = context.get("cargo", "General Goods")
        cargo_sc = context.get("cargo_score", 4.0)

        # ── Tool use: look up commodity risk on Wikipedia ──
        wiki = self.use_tool(
            "wikipedia_tool",
            log_step=f"🔧 Wikipedia lookup: {cargo} transport hazards",
            query=f"{cargo} transport risk India",
            sentences=3,
        )
        wiki_ctx = wiki["data"].get("summary", "") if wiki["success"] else ""

        prompt = f"""You are a Cargo Risk Sub-Agent in TruckShield AI, India's commercial
truck insurance system. You have been spawned by the RiskScoringAgent to analyse
one specific cargo category in depth.

Cargo Category : {cargo}
Cargo Risk Score: {cargo_sc}/10 (MoRTH/NCRB calibrated)
Wikipedia context: {wiki_ctx if wiki_ctx else "Not available."}

In 2-3 sentences:
1. Explain the primary transport hazard for this cargo type in India.
2. State whether the risk score of {cargo_sc}/10 is appropriate.
3. Recommend one mitigation measure specific to this cargo.
Actuarial and concise."""
        return self.orchestrator.call_gemini(prompt, max_tokens=180, agent_name=self.name)

    def run(self, cargo: str, cargo_score: float) -> dict:
        reasoning = self.reason({"cargo": cargo, "cargo_score": cargo_score})
        self.orchestrator.log_reasoning(self.name, "🚛 Cargo hazard analysis", reasoning)
        msg = self._emit(
            receiver="RiskScoringAgent",
            msg_type="cargo_sub_analysis",
            payload={"cargo": cargo, "cargo_score": cargo_score, "reasoning": reasoning},
            reasoning=reasoning,
        )
        self.orchestrator.deliver(msg, self.memory)
        return {"cargo": cargo, "cargo_score": cargo_score, "sub_reasoning": reasoning}


class DriverSubAgent(BaseAgent):
    """
    Sub-agent spawned by RiskScoringAgent.
    Specialises in driver behaviour and fleet management risk.
    Uses the accident_stats_tool to ground its analysis in data.
    """
    def __init__(self, orchestrator: Orchestrator):
        super().__init__("DriverSubAgent", orchestrator)

    def reason(self, context: dict) -> str:
        driver_beh  = context.get("driver_beh", 5.0)
        avg_exp     = context.get("avg_exp", 8.0)
        at_fault    = context.get("at_fault", 0)
        turnover    = context.get("turnover", 25.0)
        state       = context.get("state", "")
        route       = context.get("route", "")

        # ── Tool use: pull accident stats for context ──
        stats = self.use_tool(
            "accident_stats_tool",
            log_step="🔧 NCRB accident stats lookup",
            state=state,
            route_type=route,
        )
        stat_ctx = ""
        if stats["success"]:
            d = stats["data"]
            stat_ctx = (f"State risk index: {d.get('state_risk_index', 'N/A')}/10, "
                        f"Route risk index: {d.get('route_risk_index', 'N/A')}/10 "
                        f"({d.get('data_source', '')})")

        prompt = f"""You are a Driver Risk Sub-Agent in TruckShield AI, spawned to
analyse driver behaviour risk for a commercial truck fleet in India.

Driver Behaviour Score : {driver_beh}/10 (higher = safer)
Average Driver Experience: {avg_exp} years
At-Fault Accidents (3yr) : {at_fault}
Driver Turnover Rate     : {turnover}%/yr
Accident statistics      : {stat_ctx if stat_ctx else "Not available."}

In 2-3 sentences:
1. Assess whether the driver profile poses elevated risk relative to Indian fleet norms.
2. Identify the single largest driver-related risk factor.
3. Recommend one specific HR or safety intervention.
Actuarial and concise."""
        return self.orchestrator.call_gemini(prompt, max_tokens=180, agent_name=self.name)

    def run(self, context: dict) -> dict:
        reasoning = self.reason(context)
        self.orchestrator.log_reasoning(self.name, "👤 Driver behaviour analysis", reasoning)
        msg = self._emit(
            receiver="RiskScoringAgent",
            msg_type="driver_sub_analysis",
            payload={**context, "reasoning": reasoning},
            reasoning=reasoning,
        )
        self.orchestrator.deliver(msg, self.memory)
        return {**context, "sub_reasoning": reasoning}


class FraudScreenSubAgent(BaseAgent):
    """
    Sub-agent spawned by UnderwritingAgent when portfolio anomaly signals
    are detected (mean claims ≥ 2.5 OR mean at-fault accidents ≥ 1.5).

    WHAT IT DOES (and how it affects pricing/UW decisions):
    ───────────────────────────────────────────────────────
    1. Analyses the portfolio-level anomaly indicators.
    2. Decides whether to raise a FRAUD FLAG (boolean).
    3. If flagged: computes a fraud_loading (additional OD premium
       loading, typically 3–8%) that is injected back into _run_underwriting_core
       and then carried forward into _run_pricing_core via uw_fraud_loading column.
    4. Also upgrades individual policy decisions:
         ACCEPT  → REFER   (if policy has ≥ 2 at-fault accidents AND ≥ 3 claims)
         REFER   → DECLINE (if policy has ≥ 3 at-fault accidents AND claim amt > ₹3L)
    5. Gemini reasoning explains the fraud signals to the underwriter.

    This means: if you run with Gemini (agentic mode) and the portfolio
    triggers the anomaly threshold, FraudScreenSubAgent changes both the
    underwriting decision MIX and the final premium for affected policies —
    giving a measurably different output vs deterministic mode.
    """
    def __init__(self, orchestrator: Orchestrator):
        super().__init__("FraudScreenSubAgent", orchestrator)

    def reason(self, context: dict) -> str:
        claims_ct   = context.get("claims_ct", 0)
        claim_amt   = context.get("claim_amt", 0)
        at_fault    = context.get("at_fault", 0)
        violations  = context.get("violations", 0)
        age         = context.get("vehicle_age", 0)
        composite   = context.get("composite", 5.0)

        prompt = f"""You are a Fraud Screening Sub-Agent in TruckShield AI.
You have been spawned by the Underwriting Agent because this portfolio shows anomaly signals.

Portfolio anomaly indicators (means across all policies):
  Avg Claims Count (3yr) : {claims_ct}   | Avg Claim Amount: INR {claim_amt:,}
  Avg At-Fault Accidents : {at_fault}    | Avg Traffic Violations: {violations}
  Avg Vehicle Age        : {age} years   | Avg Composite Risk Score: {composite}/10

In 4 sentences:
1. State whether these portfolio-level indicators are consistent with claim gaming,
   organised fraud, or a genuinely high-risk fleet.
2. Identify the single most suspicious combination of indicators.
3. State your recommendation: (a) NO FRAUD FLAG — standard underwriting applies,
   (b) FRAUD FLAG — apply 3% additional OD loading to all flagged policies, or
   (c) SEVERE FRAUD FLAG — apply 7% additional OD loading + escalate to REFER/DECLINE.
4. Describe what an underwriter should look for when reviewing individual flagged policies.
Use IRDAI fraud detection guidelines. Professional, direct tone."""
        return self.orchestrator.call_gemini(prompt, max_tokens=250, agent_name=self.name)

    def run(self, context: dict) -> dict:
        reasoning = self.reason(context)
        self.orchestrator.log_reasoning(
            self.name,
        ...
        # ── Tiered fraud assessment ────────────────────────────────────
        # Severe: high claims AND high at-fault AND large amounts
        severe = (claims_ct >= 3 and at_fault >= 2 and claim_amt > 300000)
        # Moderate: elevated claims OR elevated at-fault
        moderate = (not severe) and (claims_ct >= 2.5 or at_fault >= 1.5)

        if severe:
            is_flagged    = True
            fraud_loading = 0.07   # 7% additional OD loading
            flag_level    = "SEVERE"
        elif moderate:
            is_flagged    = True
            fraud_loading = 0.03   # 3% additional OD loading
            flag_level    = "MODERATE"
        else:
            is_flagged    = False
            fraud_loading = 0.00
        ...
        result = {
            ...
            "fraud_loading": fraud_loading,   # ← fed into pricing as extra OD loading
            "sub_reasoning": reasoning,
        }

        msg = self._emit(
            receiver="UnderwritingAgent",
            msg_type="fraud_screen_result",
...
# ============================================================
# THE FIVE CONCRETE AGENTS
# Each agent: receives a message → reasons with Gemini → runs
# its actuarial logic → emits a message to the next agent.
# ============================================================

class ValidationAgent(BaseAgent):
    """
    Agent 1 — Data Validation Agent
    Validates uploaded data for nulls, out-of-range values,
    invalid categories, and date / travel-frequency logic.
    Gemini reviews the validation summary and writes a human-
    readable audit note.

    Uses weather_tool to check if the top state in the
    portfolio has any live weather anomaly that could affect
    data quality assessment. Uses retry (up to 3 attempts).
    """
    def __init__(self, orchestrator: Orchestrator):
        super().__init__("ValidationAgent", orchestrator)

    def reason(self, context: dict) -> str:
        n_rows    = context.get("n_rows", 0)
        n_errors  = context.get("n_errors", 0)
        error_cols = context.get("error_cols", [])
        top_state  = context.get("top_state", "")

        # ── Tool use: fetch live weather for the dominant state ──
        weather_ctx = ""
        if top_state:
            wx = self.use_tool(
                "weather_tool",
                log_step=f"🔧 Live weather check: {top_state}",
                location=top_state + " India",
            )
            if wx["success"] and wx["data"].get("description"):
                d = wx["data"]
                weather_ctx = (
                    f"Current conditions in {top_state}: "
                    f"Temp {d.get('temperature_c', 'N/A')}°C, "
                    f"Wind {d.get('wind_kph', 'N/A')} km/h, "
                    f"Visibility {d.get('visibility_km', 'N/A')} km. "
                    f"{d.get('description', '')}"
                )

        prompt = f"""You are the Data Validation Agent in TruckShield AI, a commercial truck
insurance pricing system for India.

You have just validated a portfolio of {n_rows} truck insurance policies.
Flagged rows: {n_errors}
Problematic columns (most frequent): {", ".join(error_cols[:5]) if error_cols else "none"}
Live weather context (dominant state): {weather_ctx if weather_ctx else "Not available."}

In 3-4 concise sentences:
1. State whether the data quality is acceptable for actuarial modelling.
2. Identify the most critical data gap if any.
3. Recommend whether to proceed or request data remediation.
4. Note any weather/visibility factor that may affect claim pattern analysis.
Keep the tone professional and actuarial."""
        # ── Retry: up to 3 attempts with backoff ──
        return self.orchestrator.call_gemini(
            prompt, max_tokens=220, max_retries=3, agent_name=self.name
        )

    def run(self, df_input):
        # (actual validation logic lives on TruckShieldEngine.validate — called via engine)
        pass   # engine.validate() calls self.agent_validation.reason() after computing stats


class ClimateAgent(BaseAgent):
    """
    Agent 2 — Climate Risk Agent
    Scores Flood, Fog, and Cyclone exposure per state + route +
    climate zone. Gemini interprets the top-risk states and
    explains why the composite score is high or low.

    Calls weather_tool to enrich its reasoning with live
    conditions. Uses retry (up to 3 attempts).
    """
    def __init__(self, orchestrator: Orchestrator):
        super().__init__("ClimateAgent", orchestrator)

    def reason(self, context: dict) -> str:
        state      = context.get("state", "N/A")
        route      = context.get("route_type", "N/A")
        climate_zn = context.get("climate_zone", "N/A")
        flood      = context.get("flood", 0)
        fog        = context.get("fog", 0)
        cyclone    = context.get("cyclone", 0)
        composite  = context.get("climate_composite", 0)

        # ── Tool use: live weather for the top state ──
        wx = self.use_tool(
            "weather_tool",
            log_step=f"🔧 Live weather for climate scoring: {state}",
            location=state + " India",
        )
        wx_ctx = ""
        if wx["success"] and wx["data"].get("wind_kph") is not None:
            d = wx["data"]
            wx_ctx = (f"Live: Temp {d.get('temperature_c')}°C, "
                      f"Wind {d.get('wind_kph')} km/h, "
                      f"Visibility {d.get('visibility_km')} km. "
                      f"{d.get('description', '')}")

        prompt = f"""You are the Climate Risk Agent in TruckShield AI, a commercial truck
insurance pricing system for India.

You have scored climate risk for a portfolio sample with the following profile:
  State: {state} | Route: {route} | Climate Zone: {climate_zn}
  Flood score: {flood}/10 | Fog score: {fog}/10 | Cyclone score: {cyclone}/10
  Climate Composite (0.38*Flood + 0.22*Fog + 0.25*Cyclone + 0.15*Route): {composite}/10
  Live weather data: {wx_ctx if wx_ctx else "Not available."}

In 3-4 sentences explain:
1. Which climate peril dominates and why (based on NDMA/IMD data).
2. How this composite score should influence the underwriting decision.
3. Any seasonal or geographic nuance relevant to this state/route combination.
4. Whether the live weather data corroborates the model risk scores.
Keep the tone professional and actuarial."""
        return self.orchestrator.call_gemini(
            prompt, max_tokens=220, max_retries=3, agent_name=self.name
        )

    def run(self, df: "pd.DataFrame", engine: "TruckShieldEngine") -> "pd.DataFrame":
        return engine._run_climate_core(df)


class RiskScoringAgent(BaseAgent):
    """
    Agent 3 — Risk Scoring Agent
    Builds 5 sub-scores (Cargo, Exposure, Safety, Driver Behavior,
    Night Travel) and computes Accident Probability and Composite
    Risk Score. Gemini explains the top risk drivers.

    Spawns CargoSubAgent and DriverSubAgent for specialised
    sub-analysis. Uses accident_stats_tool directly. Retry x3.
    """
    def __init__(self, orchestrator: Orchestrator):
        super().__init__("RiskScoringAgent", orchestrator)

    def reason(self, context: dict) -> str:
        composite   = context.get("mean_composite", 0)
        cargo       = context.get("mean_cargo", 0)
        exposure    = context.get("mean_exposure", 0)
        safety      = context.get("mean_safety", 0)
        driver_beh  = context.get("mean_driver_beh", 0)
        acc_prob    = context.get("mean_acc_prob", 0)
        top_cargo   = context.get("top_cargo", "N/A")
        top_route   = context.get("top_route", "N/A")
        top_state   = context.get("top_state", "")

        # ── Tool use: accident stats for top state/route ──
        stats = self.use_tool(
            "accident_stats_tool",
            log_step=f"🔧 NCRB accident stats: {top_state} / {top_route}",
            state=top_state,
            route_type=top_route,
        )
        stat_ctx = ""
        if stats["success"]:
            d = stats["data"]
            stat_ctx = (f"NCRB combined risk index {d.get('combined_index')}/10 "
                        f"(state {d.get('state_risk_index')}, route {d.get('route_risk_index')})")

        #Spawn CargoSubAgent for in-depth cargo analysis ──
        cargo_sub_result = {}
        driver_sub_result = {}
        if self.orchestrator._gemini_key:
            try:
                cargo_sub = self.spawn_sub(CargoSubAgent)
                cargo_sub_result = cargo_sub.run(cargo=top_cargo, cargo_score=round(cargo, 2))
                # Store on orchestrator so UI can display impact explainer
                self.orchestrator._cargo_sub_result = cargo_sub_result

                driver_sub = self.spawn_sub(DriverSubAgent)
                driver_sub_result = driver_sub.run({
                    "driver_beh": round(driver_beh, 2),
                    "avg_exp":    context.get("mean_avg_exp", 8.0),
                    "at_fault":   context.get("mean_at_fault", 0),
                    "turnover":   context.get("mean_turnover", 25.0),
                    "state":      top_state,
                    "route":      top_route,
                })
                # Store on orchestrator so UI can display impact explainer
                self.orchestrator._driver_sub_result = driver_sub_result
            except Exception:
                pass  # sub-agent failure doesn't block the parent

        sub_cargo_note  = cargo_sub_result.get("sub_reasoning", "")[:150] if cargo_sub_result else ""
        sub_driver_note = driver_sub_result.get("sub_reasoning", "")[:150] if driver_sub_result else ""

        prompt = f"""You are the Risk Scoring Agent in TruckShield AI, a commercial truck
insurance pricing system for India (MoRTH/NCRB calibrated model).

Portfolio mean scores (0-10 scale):
  Composite Risk : {composite:.2f}
  Cargo Risk     : {cargo:.2f}   (top category: {top_cargo})
  Risk Exposure  : {exposure:.2f}  (GVW + age + route + overloading)
  Safety Score   : {safety:.2f}   (higher = safer; BS norm + fuel + ADAS)
  Driver Behavior: {driver_beh:.2f}  (experience - violations - turnover)
  Accident Prob. : {acc_prob:.2f}   (top route type: {top_route})
Accident stats tool: {stat_ctx if stat_ctx else "Not available."}
Cargo sub-agent note: {sub_cargo_note if sub_cargo_note else "Not run."}
Driver sub-agent note: {sub_driver_note if sub_driver_note else "Not run."}

In 4 sentences:
1. Identify the 2 strongest risk drivers incorporating the sub-agent findings.
2. Explain how the composite score was calculated (weights).
3. Compare this portfolio to a typical Indian commercial truck fleet.
4. Recommend one specific risk mitigation measure to reduce the composite score.
Professional, actuarial tone."""
        return self.orchestrator.call_gemini(
            prompt, max_tokens=280, max_retries=3, agent_name=self.name
        )

    def run(self, df: "pd.DataFrame", engine: "TruckShieldEngine") -> "pd.DataFrame":
        return engine._run_risk_scoring_core(df)


class UnderwritingAgent(BaseAgent):
    """
    Agent 4 — Underwriting Agent
    Makes Accept / Refer / Decline decisions based on Composite Risk
    Score, vehicle age, cargo type, and permit status. Applies risk
    and claims-history loading to OD premium. Gemini justifies the
    decision mix and flags systemic portfolio concerns.

    Spawns FraudScreenSubAgent when anomaly signals are detected
    (high claims + high at-fault). Uses Wikipedia for regulatory context.
    Retry x3.
    """
    def __init__(self, orchestrator: Orchestrator):
        super().__init__("UnderwritingAgent", orchestrator)

    def reason(self, context: dict) -> str:
        accept  = context.get("n_accept", 0)
        refer   = context.get("n_refer", 0)
        decline = context.get("n_decline", 0)
        total   = accept + refer + decline or 1
        mean_loading = context.get("mean_loading_pct", 0)
        top_decline_reason = context.get("top_decline_reason", "high composite risk + old vehicle age")
        mean_claims    = context.get("mean_claims_ct", 0)
        mean_at_fault  = context.get("mean_at_fault", 0)
        mean_claim_amt = context.get("mean_claim_amt", 0)
        mean_vehicle_age = context.get("mean_vehicle_age", 5)
        mean_composite   = context.get("mean_composite", 5.0)

        # -- Tool use: Wikipedia for IRDAI regulatory context --
        wiki = self.use_tool(
            "wikipedia_tool",
            log_step="[TOOL] Wikipedia: IRDAI motor insurance regulations",
            query="IRDAI commercial vehicle insurance India regulations",
            sentences=3,
        )
        irdai_ctx = wiki["data"].get("summary", "") if wiki["success"] else ""

        #Spawn FraudScreenSubAgent if anomaly signals detected --
        fraud_note = ""
        self.orchestrator._fraud_screen_result = {}   # reset before each run
        anomaly_trigger = (mean_claims >= 2.5 or mean_at_fault >= 1.5)
        if anomaly_trigger and self.orchestrator._gemini_key:
            try:
                fraud_sub = self.spawn_sub(FraudScreenSubAgent)
                fraud_result = fraud_sub.run({
                    "claims_ct":   round(mean_claims, 1),
                    "claim_amt":   int(mean_claim_amt),
                    "at_fault":    round(mean_at_fault, 1),
                    "violations":  context.get("mean_violations", 0),
                    "vehicle_age": round(mean_vehicle_age, 1),
                    "composite":   round(mean_composite, 2),
                })
                # Store result on orchestrator so _run_underwriting_core can read it
                self.orchestrator._fraud_screen_result = fraud_result
                fraud_loading_pct = round(fraud_result.get("fraud_loading", 0) * 100, 0)
                fraud_note = fraud_result.get("sub_reasoning", "")[:200]
                if fraud_result.get("flagged"):
                    flag_level = fraud_result.get("flag_level", "")
                    fraud_note = (f"⚠️ {flag_level} FRAUD FLAG — {fraud_loading_pct:.0f}% "
                                  f"additional OD loading applied to flagged policies. ") + fraud_note
            except Exception:
                pass

        prompt = f"""You are the Underwriting Agent in TruckShield AI, a commercial truck
insurance pricing system for India (IRDAI regulatory framework).

Portfolio underwriting decisions:
  ACCEPT  : {accept} ({accept/total*100:.1f}%)
  REFER   : {refer}  ({refer/total*100:.1f}%)
  DECLINE : {decline} ({decline/total*100:.1f}%)
  Mean extra UW loading applied: {mean_loading:.2f}%
  Primary decline trigger: {top_decline_reason}
IRDAI regulatory context: {irdai_ctx[:200] if irdai_ctx else "Not available."}
Fraud screening sub-agent: {fraud_note if fraud_note else "Not triggered (anomaly threshold not met)."}

In 4 sentences:
1. Assess whether the ACCEPT/REFER/DECLINE mix is appropriate for a
   commercial truck fleet in India.
2. Identify any systemic portfolio concern (e.g. too many declines, under-loading).
3. Justify the loading level relative to IRDAI guidelines.
4. Recommend one underwriting rule adjustment if needed, considering fraud signals.
Professional, actuarial, IRDAI-aware tone."""
        return self.orchestrator.call_gemini(
            prompt, max_tokens=280, max_retries=3, agent_name=self.name
        )

    def run(self, df: "pd.DataFrame", engine: "TruckShieldEngine") -> "pd.DataFrame":
        return engine._run_underwriting_core(df)


class PricingAgent(BaseAgent):
    """
    Agent 5 — Pricing Agent
    Uses XGBoost frequency x severity models to predict claim
    behaviour. Computes TP premium (IRDAI GVW-slab tariff) and
    OD premium (2.5% base rate x relativity x accident load /
    expense denom). Applies NCB discount, premium floor, and
    produces the final gross & pure premiums for every policy.
    Gemini summarises the final pricing output and validates it
    against market benchmarks.

    Uses wikipedia_tool to pull actuarial/XGBoost context,
    and accident_stats_tool for benchmark grounding. Retry x3.
    """
    def __init__(self, orchestrator: Orchestrator):
        super().__init__("PricingAgent", orchestrator)

    def reason(self, context: dict) -> str:
        mean_total_gross = context.get("mean_total_gross", 0)
        mean_tp_gross    = context.get("mean_tp_gross", 0)
        mean_od_gross    = context.get("mean_od_gross", 0)
        mean_pure        = context.get("mean_total_pure", 0)
        freq_mae         = context.get("freq_mae", 0)
        sev_mae          = context.get("sev_mae", 0)
        top_state        = context.get("top_state", "")
        top_route        = context.get("top_route", "")
        loss_ratio_implied = (mean_pure / mean_total_gross * 100) if mean_total_gross else 0

        # -- Tool use: XGBoost background for model commentary --
        wiki = self.use_tool(
            "wikipedia_tool",
            log_step="[TOOL] Wikipedia: XGBoost actuarial modelling",
            query="XGBoost gradient boosting insurance pricing",
            sentences=3,
        )
        xgb_ctx = wiki["data"].get("summary", "") if wiki["success"] else ""

        # -- Tool use: accident stats for benchmark grounding --
        stats = self.use_tool(
            "accident_stats_tool",
            log_step=f"[TOOL] NCRB benchmark: {top_state}/{top_route}",
            state=top_state,
            route_type=top_route,
        )
        benchmark_ctx = ""
        if stats["success"]:
            d = stats["data"]
            benchmark_ctx = (f"NCRB combined risk index {d.get('combined_index')}/10 "
                             f"for {top_state}/{top_route}")

        prompt = f"""You are the Pricing Agent in TruckShield AI, a commercial truck
insurance pricing system for India (IRDAI compliant).

Portfolio pricing output:
  Mean Total Gross Premium : INR {mean_total_gross:,.0f}
  Mean TP Gross (IRDAI tariff) : INR {mean_tp_gross:,.0f}
  Mean OD Gross (insurer-priced): INR {mean_od_gross:,.0f}
  Mean Total Pure (loss cost)   : INR {mean_pure:,.0f}
  Implied Combined Loss Ratio   : {loss_ratio_implied:.1f}%
  XGBoost Frequency MAE : {freq_mae:.4f} claims/yr
  XGBoost Severity MAE  : INR {sev_mae:,.0f}
XGBoost context: {xgb_ctx[:200] if xgb_ctx else "Not available."}
NCRB benchmark: {benchmark_ctx if benchmark_ctx else "Not available."}

In 4 sentences:
1. Validate whether the implied loss ratio is reasonable for Indian commercial trucks.
2. Assess whether the OD:TP premium split looks market-consistent.
3. Comment on model accuracy (MAE values) relative to XGBoost norms.
4. Identify one pricing risk or caveat the underwriting team should be aware of.
Professional, actuarial tone. Currency in INR."""
        return self.orchestrator.call_gemini(
            prompt, max_tokens=280, max_retries=3, agent_name=self.name
        )

    def run(self, df: "pd.DataFrame", engine: "TruckShieldEngine") -> "pd.DataFrame":
        return engine._run_pricing_core(df)

class TruckShieldEngine:
    """
    Main engine — now a true Agentic AI system.

    Architecture
    ────────────
    Orchestrator coordinates 5 autonomous Agent objects.
    Each Agent:
      1. receives an AgentMessage from the previous agent via the Orchestrator
      2. calls its own reason() method — sends a prompt to Gemini and gets
         a natural-language justification for its decision
      3. executes its deterministic actuarial computation (run())
      4. emits an AgentMessage to the next agent carrying both the data
         payload AND the Gemini reasoning text

    The full reasoning chain is stored in orchestrator.reasoning_chain and
    displayed in the "🤖 Agent Reasoning Chain" tab.

    Without a Gemini API key the system falls back to pure deterministic
    mode— every tab still works.
    """
    ...
    def __init__(self):
        ...
        # ── 5 autonomous agent objects ───────────────────────
        self.agent_validation   = ValidationAgent(self.orchestrator)
        self.agent_climate      = ClimateAgent(self.orchestrator)
        self.agent_risk         = RiskScoringAgent(self.orchestrator)
        self.agent_underwriting = UnderwritingAgent(self.orchestrator)
        self.agent_pricing      = PricingAgent(self.orchestrator)
    ...
    # ║  ValidationAgent.reason() calls Gemini to write an  ║
    # ║  audit note. AgentMessage carries result to ClimateAgent.       ║
    # ╚══════════════════════════════════════════════════════════════════╝
    def validate(self, df: pd.DataFrame):
        ...
        # ── AGENTIC STEP: Agent 1 — compute context now, but defer logging until
        #    train_and_process() so the entry survives orchestrator.reset() ──
        error_col_counts: dict = {}
        for cell in error_cells:
            error_col_counts[cell["column"]] = error_col_counts.get(cell["column"], 0) + 1
        top_error_cols = sorted(error_col_counts, key=error_col_counts.get, reverse=True)[:5]

        # Store the validation context on self so train_and_process can re-call
        # agent_validation.reason() AFTER the orchestrator reset.
        self._validation_context = {
            "n_rows": len(df), "n_errors": len(error_rows), "error_cols": top_error_cols,
            "top_state": df["state"].mode()[0] if "state" in df.columns and len(df) > 0 else "",
        }

        return df, error_rows, detail_df, log

    def set_gemini_key(self, key: str):
        """Pass Gemini API key into the Orchestrator for agent reasoning."""
        self.orchestrator.set_gemini_key(key)

    def set_llm_model(self, model_string: str):
        """Pass the selected LLM model name into the Orchestrator."""
        self.orchestrator.set_llm_model(model_string)

    def train_and_process(self, df_full: pd.DataFrame, error_rows: set, fast_mode: bool = True):
        # Reset orchestrator so the reasoning chain is fresh for this run
        self.orchestrator.reset()
        # Re-apply Gemini key and model selection from session state (survives page reruns)
        try:
            import streamlit as _st
            _key = _st.session_state.get("gemini_key", "")
            if _key:
                self.orchestrator.set_gemini_key(_key)
            _model = _st.session_state.get("llm_model", "gemini-2.5-flash")
            self.orchestrator.set_llm_model(_model)
        except Exception:
            pass
        ...
        # ── Agent 1 — Validation: log reasoning NOW (after reset, so it persists) ──
        _val_ctx = getattr(self, "_validation_context", {
            "n_rows": len(df_clean), "n_errors": 0, "error_cols": [],
            "top_state": df_clean["state"].mode()[0] if "state" in df_clean.columns and len(df_clean) > 0 else "",
        })
        _val_reasoning = self.agent_validation.reason(_val_ctx)
        self.orchestrator.log_reasoning("Agent 1 — Validation", "Data Quality Audit", _val_reasoning)
        _val_out_msg = self.agent_validation._emit(
            receiver="ClimateAgent",
            msg_type="validation_complete",
            payload={"n_rows": _val_ctx["n_rows"], "n_errors": _val_ctx["n_errors"],
                     "error_cols": _val_ctx["error_cols"]},
            reasoning=_val_reasoning,
        )
        self.orchestrator.deliver(_val_out_msg, self.agent_climate.memory)
    ...
    # ║  ClimateAgent.reason() calls Gemini to explain the  ║
    # ║  dominant peril. AgentMessage carries result to RiskScoringAgent║
    # ╚══════════════════════════════════════════════════════════════════╝
    def run_climate(self, df: pd.DataFrame) -> pd.DataFrame:
        df_c = self._run_climate_core(df)

        # ── AGENTIC STEP: Agent 2 reasons with Gemini, emits message to Agent 3 ──
        # Sample the modal state/route/zone for the reasoning context
        sample_state  = df_c["state"].mode()[0] if "state" in df_c.columns else "N/A"
        sample_route  = df_c["route_type"].mode()[0] if "route_type" in df_c.columns else "N/A"
        sample_zone   = df_c["climate_zone"].mode()[0] if "climate_zone" in df_c.columns else "N/A"
        mean_flood    = round(df_c["climate_flood_score"].mean(), 2)
        mean_fog      = round(df_c["climate_fog_score"].mean(), 2)
        mean_cyclone  = round(df_c["climate_cyclone_score"].mean(), 2)
        mean_comp     = round(df_c["climate_composite"].mean(), 2)

        reasoning = self.agent_climate.reason({
            "state": sample_state, "route_type": sample_route, "climate_zone": sample_zone,
            "flood": mean_flood, "fog": mean_fog, "cyclone": mean_cyclone,
            "climate_composite": mean_comp,
        })
        self.orchestrator.log_reasoning("Agent 2 — Climate Risk", "Peril Exposure Analysis", reasoning)

        out_msg = self.agent_climate._emit(
            receiver="RiskScoringAgent",
            msg_type="climate_scores",
            payload={"mean_flood": mean_flood, "mean_fog": mean_fog,
                     "mean_cyclone": mean_cyclone, "mean_composite": mean_comp},
            reasoning=reasoning,
        )
        self.orchestrator.deliver(out_msg, self.agent_risk.memory)
        return df_c

    def _run_climate_core(self, df: pd.DataFrame) -> pd.DataFrame:
    ...
    # ║  RiskScoringAgent.reason() calls Gemini to explain  ║
    # ║  the top risk drivers. Message carries result to UW Agent.      ║
    # ╚══════════════════════════════════════════════════════════════════╝
    def run_risk_scoring(self, df: pd.DataFrame) -> pd.DataFrame:
        df_r = self._run_risk_scoring_core(df)

        # ── AGENTIC STEP: Agent 3 reasons with Gemini, emits message to Agent 4 ──
        mean_composite = round(df_r["composite_risk_score"].mean(), 2)
        mean_cargo     = round(df_r["cargo_risk_score"].mean(), 2)
        mean_exposure  = round(df_r["risk_exposure_score"].mean(), 2)
        mean_safety    = round(df_r["safety_score"].mean(), 2)
        mean_driver    = round(df_r["driver_behavior_score"].mean(), 2)
        mean_acc       = round(df_r["accident_probability"].mean(), 2)
        top_cargo      = df_r["goods_category"].mode()[0] if "goods_category" in df_r.columns else "N/A"
        top_route      = df_r["route_type"].mode()[0] if "route_type" in df_r.columns else "N/A"

        reasoning = self.agent_risk.reason({
            "mean_composite": mean_composite, "mean_cargo": mean_cargo,
            "mean_exposure": mean_exposure, "mean_safety": mean_safety,
            "mean_driver_beh": mean_driver, "mean_acc_prob": mean_acc,
            "top_cargo": top_cargo, "top_route": top_route,
            "top_state":      df_r["state"].mode()[0] if "state" in df_r.columns else "",
            "mean_avg_exp":   round(df_r["average_driver_experience"].mean(), 1) if "average_driver_experience" in df_r.columns else 8.0,
            "mean_at_fault":  round(df_r["at_fault_accidents"].mean(), 2) if "at_fault_accidents" in df_r.columns else 0,
            "mean_turnover":  round(df_r["driver_turnover_rate"].mean(), 1) if "driver_turnover_rate" in df_r.columns else 25.0,
        })
        self.orchestrator.log_reasoning("Agent 3 — Risk Scoring", "Composite Risk Analysis", reasoning)

        out_msg = self.agent_risk._emit(
            receiver="UnderwritingAgent",
            msg_type="risk_scores",
            payload={"mean_composite": mean_composite, "mean_acc_prob": mean_acc,
                     "top_cargo": top_cargo, "top_route": top_route},
            reasoning=reasoning,
        )
        self.orchestrator.deliver(out_msg, self.agent_underwriting.memory)
        return df_r

    def _run_risk_scoring_core(self, df: pd.DataFrame) -> pd.DataFrame:
        df_r = df.copy()
    ...
    # ║  UnderwritingAgent.reason() calls Gemini to justify  ║
    # ║  the decision mix. Message carries result to PricingAgent.      ║
    # ╚══════════════════════════════════════════════════════════════════╝
    def run_underwriting(self, df: pd.DataFrame) -> pd.DataFrame:
        df_u = self._run_underwriting_core(df)

        # ── AGENTIC STEP: Agent 4 reasons with Gemini, emits message to Agent 5 ──
        n_accept  = int((df_u["uw_decision"] == "ACCEPT").sum())
        n_refer   = int((df_u["uw_decision"] == "REFER").sum())
        n_decline = int((df_u["uw_decision"] == "DECLINE").sum())
        mean_load = round(df_u["uw_extra_loading"].mean() * 100, 2)

        # Identify primary decline trigger
        declined = df_u[df_u["uw_decision"] == "DECLINE"]
        if not declined.empty:
            old_vehicles = int((declined["vehicle_age_yrs"] > 17).sum())
            high_risk    = int((declined["composite_risk_score"] > 8.5).sum())
            top_reason   = (f"high composite risk (>8.5) in {high_risk} cases, "
                            f"old vehicles (>17 yr) in {old_vehicles} cases")
        else:
            top_reason = "no declines in this portfolio"

        reasoning = self.agent_underwriting.reason({
            "n_accept": n_accept, "n_refer": n_refer, "n_decline": n_decline,
            "mean_loading_pct": mean_load, "top_decline_reason": top_reason,
            "mean_claims_ct":   round(df_u["total_claims_count"].mean(), 2) if "total_claims_count" in df_u.columns else 0,
            "mean_at_fault":    round(df_u["at_fault_accidents"].mean(), 2) if "at_fault_accidents" in df_u.columns else 0,
            "mean_claim_amt":   round(df_u["total_claim_amount_paid"].mean(), 0) if "total_claim_amount_paid" in df_u.columns else 0,
            "mean_violations":  round(df_u["traffic_violations"].mean(), 1) if "traffic_violations" in df_u.columns else 0,
            "mean_vehicle_age": round(df_u["vehicle_age_yrs"].mean(), 1) if "vehicle_age_yrs" in df_u.columns else 5,
            "mean_composite":   round(df_u["composite_risk_score"].mean(), 2) if "composite_risk_score" in df_u.columns else 5.0,
        })
        self.orchestrator.log_reasoning("Agent 4 — Underwriting", "UW Decision Justification", reasoning)

        out_msg = self.agent_underwriting._emit(
            receiver="PricingAgent",
            msg_type="uw_decisions",
            payload={"n_accept": n_accept, "n_refer": n_refer, "n_decline": n_decline,
                     "mean_loading_pct": mean_load},
            reasoning=reasoning,
        )
        self.orchestrator.deliver(out_msg, self.agent_pricing.memory)
        return df_u

    def _run_underwriting_core(self, df: pd.DataFrame) -> pd.DataFrame:
        df_u = df.copy()
        decisions, loadings, claim_loadings, fraud_loadings = [], [], [], []

        # ── Read FraudScreenSubAgent result (set by UnderwritingAgent.reason()) ──
        fraud_result   = getattr(self.orchestrator, "_fraud_screen_result", {})
        fraud_flagged  = fraud_result.get("flagged", False)
        fraud_loading  = fraud_result.get("fraud_loading", 0.0)   # e.g. 0.03 or 0.07
        fraud_level    = fraud_result.get("flag_level", "NONE")
        ...
        for _, row in df_u.iterrows():
            ...
            # ── FraudScreenSubAgent influence: upgrade individual policy decisions ──
            # Only applied when sub-agent was spawned AND flagged the portfolio
            policy_fraud_load = 0.0
            if fraud_flagged:
                policy_fraud_load = fraud_loading
                if fraud_level == "SEVERE":
                    # Severe: ACCEPT→REFER if ≥2 at-fault + ≥3 claims
                    # REFER→DECLINE if ≥3 at-fault + claim amount > ₹3L
                    at_f = row.get("at_fault_accidents", 0)
                    c_ct = row.get("total_claims_count", 0)
                    c_am = row.get("total_claim_amount_paid", 0)
                    if decision == "REFER" and at_f >= 3 and c_am > 300000:
                        decision = "DECLINE"
                        loading  = 0
                    elif decision == "ACCEPT" and at_f >= 2 and c_ct >= 3:
                        decision = "REFER"
                        loading  = round(loading + fraud_loading, 3)
                elif fraud_level == "MODERATE":
                    # Moderate: ACCEPT→REFER only for severe individual outliers
                    at_f = row.get("at_fault_accidents", 0)
                    c_ct = row.get("total_claims_count", 0)
                    if decision == "ACCEPT" and at_f >= 3 and c_ct >= 4:
                        decision = "REFER"
                        loading  = round(loading + fraud_loading, 3)
            ...
            fraud_loadings.append(policy_fraud_load)

        df_u["uw_decision"]       = decisions
        df_u["uw_extra_loading"]  = loadings
        df_u["uw_claims_loading"] = claim_loadings
        df_u["uw_fraud_loading"]  = fraud_loadings   # ← NEW: from FraudScreenSubAgent
    ...
    # ║  expense denom). Applies NCB discount, premium floor, and       ║
    # ║  produces the final gross & pure premiums for every policy.     ║
    # ║  PricingAgent.reason() calls Gemini to validate the ║
    # ║  pricing output against market benchmarks.                      ║
    # ╚══════════════════════════════════════════════════════════════════╝
    def run_pricing(self, df: pd.DataFrame) -> pd.DataFrame:
        df_p = self._run_pricing_core(df)

        # ── AGENTIC STEP: Agent 5 reasons with Gemini (final reasoning step) ──
        accepted = df_p[df_p["uw_decision"] != "DECLINE"]
        if len(accepted) > 0:
            mean_total_gross = round(accepted["total_gross_premium"].mean(), 0)
            mean_tp_gross    = round(accepted["tp_gross_premium_compulsory"].mean(), 0)
            mean_od_gross    = round(accepted["od_gross_premium_optional"].mean(), 0)
            mean_pure        = round(accepted["total_pure_premium"].mean(), 0)
        else:
            mean_total_gross = mean_tp_gross = mean_od_gross = mean_pure = 0

        reasoning = self.agent_pricing.reason({
            "mean_total_gross": mean_total_gross,
            "mean_tp_gross":    mean_tp_gross,
            "mean_od_gross":    mean_od_gross,
            "mean_total_pure":  mean_pure,
            "freq_mae":         self.model_metrics.get("freq_mae", 0),
            "sev_mae":          self.model_metrics.get("sev_mae", 0),
            "top_state":  df_p["state"].mode()[0] if "state" in df_p.columns and len(df_p) > 0 else "",
            "top_route":  df_p["route_type"].mode()[0] if "route_type" in df_p.columns and len(df_p) > 0 else "",
        })
        self.orchestrator.log_reasoning("Agent 5 — Pricing", "Premium Validation & Market Benchmark", reasoning)

        # Final message — Orchestrator stores it; no further agent to deliver to
        out_msg = self.agent_pricing._emit(
            receiver="Orchestrator",
            msg_type="pricing_complete",
            payload={"mean_total_gross": mean_total_gross, "mean_od_gross": mean_od_gross},
            reasoning=reasoning,
        )
        self.orchestrator.message_log.append(out_msg)
        return df_p

    def _run_pricing_core(self, df: pd.DataFrame) -> pd.DataFrame:
        df_p = df.copy()
        ...
        def price_row(row):
            ...
            total_loading = (row["uw_extra_loading"]
                             + row.get("uw_claims_loading", 0)
                             + row.get("uw_fraud_loading", 0))   # ← FraudScreenSubAgent loading
    ...
    def export_full_dataset_bytes(self) -> bytes:
        cols = [
            ...
            "total_claims_count", "total_claim_amount_paid",
            "uw_decision", "uw_extra_loading", "uw_claims_loading", "uw_fraud_loading",
...
Version     : (current) — Full Agentic AI Architecture
Type        : Multi-Agent Actuarial & ML Pipeline with Agentic AI (Gemini reasoning per agent)
...
ARCHITECTURE — FIVE AUTONOMOUS AGENTS + ORCHESTRATOR:
  Orchestrator  — routes AgentMessages, logs reasoning chain, coordinates pipeline
  Agent 1 — Data Validation Agent   : Validates data → reasons with Gemini → emits to Agent 2
  Agent 2 — Climate Risk Agent       : Scores Flood/Fog/Cyclone → reasons with Gemini → emits to Agent 3
  Agent 3 — Risk Scoring Agent       : Composite risk score → reasons with Gemini → emits to Agent 4
  Agent 4 — Underwriting Agent       : Accept/Refer/Decline → reasons with Gemini → emits to Agent 5
  Agent 5 — Pricing Agent            : XGBoost frequency × severity → reasons with Gemini → final output

AI ASSISTANT:
  Powered by Google Gemini (model selected by user in sidebar — see ACTIVE LLM MODEL section).
...
WHY GEMINI FOR THE AI ASSISTANT?
  Google Gemini models were chosen because they are state-of-the-art, fast, and
  cost-effective large language models with a simple Python SDK (google-generativeai).
  They have strong reasoning over structured actuarial prompts, support long context
  windows (needed to inject full pipeline summaries), and require only a single
...
- Risk Scoring Agent: Cargo Risk, Exposure (GVW+age+route+overloading), Safety (BS+fuel+fatigue monitor),
  ...
  Accident Probability (NCRB/MoRTH additive model). Telematics replaced by Driver Behavior.
...
def get_chat_response(engine, user_message: str, api_key: str, history: list) -> str:
    # google-genai is an optional dependency — only imported here when the user
    # actually has a Gemini key. This keeps the base app working without installing it.
    from google import genai as genai_new
    from google.genai import types as genai_types

    # ── Which model is active? ──
    active_model_api  = engine.orchestrator._llm_model           # e.g. "gemini-2.5-flash"
    active_model_name = engine.orchestrator.get_llm_display_name() # e.g. "Google Gemini 2.5 Flash"
    ...
    if question_type == "technical_external":
        ...
        # If Wikipedia lookup fails, proceed without it — model still has embedded knowledge
    ...
    # ── Append active model info so the assistant can answer "which LLM are you using?" ──
    all_models = list(engine.orchestrator._MODEL_DISPLAY_NAMES.values())
    other_models = [m for m in all_models if m != active_model_name]
    system_prompt += f"""

ACTIVE LLM MODEL (important — use this when asked which model you are using):
  API string   : {active_model_api}
  Display name : {active_model_name}
  Selected by  : End user via the "LLM Model" selector in the sidebar
  Default      : Google Gemini 2.5 Flash
  All available options: {', '.join(all_models)}

If the user asks which LLM model you are using, which AI model powers you, or anything
similar, answer clearly: "I am currently powered by {active_model_name}. The user
selected this model from the LLM Model selector in the sidebar. The other available
options are: {', '.join(other_models)}."
"""

    # ── Step 5: Call the selected Gemini model ──
    client      = genai_new.Client(api_key=api_key)
    full_prompt = f"{system_prompt}\n\nUSER: {user_message}\n\nYour response:"
    response    = client.models.generate_content(
        model=active_model_api,   # ← uses selected model
        contents=full_prompt,
    )
    answer     = response.text.strip()

    # ── Step 6: RAG hallucination check on the AI Assistant response ──
    try:
        rag_result  = RAG_KB.check_hallucination(gemini_text=answer, query=user_message)
        status_icon = {"VERIFIED": "🟢", "CAUTION": "🟡", "FLAGGED": "🔴"}.get(
            rag_result["status"], "⚪"
        )
        if rag_result["status"] != "VERIFIED":
            evidence_block = "\n".join(
                f"- {e[:200]}" for e in rag_result["evidence"][:2]
            )
            rag_footer = (
                f"\n\n---\n{status_icon} **RAG Verification Note ({rag_result['status']}):** "
                f"{rag_result['flag_reason']}\n\n"
                f"*Retrieved domain evidence used for verification:*\n{evidence_block}"
            )
            answer = answer + rag_footer
    except Exception:
        pass   # RAG must never block the assistant
...
with st.sidebar:
    ...
    # ── LLM MODEL SELECTOR (above all other sections) ──────────────
    ...
    selected_llm_display = st.radio(
        "Choose the LLM model used by all agents and the AI Assistant:",
        options=list(LLM_OPTIONS.keys()),
        index=0,          # default: Gemini 2.5 Flash
        help=(
            "**Google Gemini 2.5 Flash** — Stable GA. Strong thinking-model reasoning, "
            "great for actuarial analysis. Recommended default.\n\n"
            "**Google Gemini 3.1 Flash Lite** — Fast & low-cost. Gemini 3 series, "
            "good for high-volume runs where speed matters more than depth.\n\n"
            "**Google Gemini 3.1 Pro Preview** — Most capable in the 3.1 family. "
            "Deepest reasoning, best for complex underwriting analysis. "
            "Preview model — may have stricter rate limits.\n\n"
            "**Google Gemini 3.5 Flash** — Latest GA frontier model (May 2026). "
            "Best agentic and coding performance; ideal for the full agentic pipeline."
        ),
    )
    selected_llm_api = LLM_OPTIONS[selected_llm_display]
    # Store in session state so it persists across reruns and is readable by train_and_process
    if st.session_state.get("llm_model") != selected_llm_api:
        st.session_state["llm_model"] = selected_llm_api
        engine.set_llm_model(selected_llm_api)
    else:
        engine.set_llm_model(selected_llm_api)   # always sync on rerun

    st.caption(f"✅ Active model: **{selected_llm_display}**")
    st.markdown("---")
    ...
    # ── Gemini API key — must be entered BEFORE clicking Run ──────────────────
    st.markdown("#### 🔑 Gemini API Key (required for AI reasoning & RAG)")
    gemini_key = st.text_input(
        "Gemini API key", type="password",
        help="Get one free at https://aistudio.google.com/apikey — required for agent reasoning and RAG hallucination checking.",
        key="gemini_key_input",
    )
    if gemini_key:
        st.session_state["gemini_key"] = gemini_key
        engine.set_gemini_key(gemini_key)   # wire key into Orchestrator
        st.caption("✅ API key active — pipeline will run in AI mode with RAG verification.")
    else:
        # Restore from session_state if already entered earlier (page rerun)
        _saved_key = st.session_state.get("gemini_key", "")
        if _saved_key:
            engine.set_gemini_key(_saved_key)
            st.caption("✅ API key restored from session — AI mode active.")
        else:
            st.caption("⚠️ No API key — pipeline runs in deterministic mode (no RAG verification).")
    st.markdown("---")
    ...
    st.markdown(
        ...
        "This is a **full Agentic AI** system: five autonomous Agent objects communicate via "
        "typed **AgentMessages** through an **Orchestrator**. Each agent calls **Gemini** to "
        "reason about its outputs before passing results to the next agent. "
        "The full reasoning chain is visible in the **🤖 Agent Reasoning Chain** tab."
    )

    st.markdown("---")
    st.markdown("### 5. About Developer Team")
    ...
    # ==========================================
    st.markdown("---")
    st.markdown("### 6. Advanced Version")

    st.link_button(
        "🔗 Go To Simpler Version",
        url="https://commercial-truck-insurance-pricing-underwriting-analysis-team5.streamlit.app/",
        type="primary",
...
st.caption(
        "⚠️ **Note:** It is strongly advised to free tier Gemini API Key Users for not to use this Advanced Version "
        "because it can exhaust your free tier Gemini API Key Limit within a single click.\n\n"
        "**Reason-** In this advance version each Agent get there own thinking ability to transfer/calculate the output. "
...
st.caption(
    "🚨 **About RAG Verification Results —** The RAG (Retrieval-Augmented Generation) guard "
    "checks every Gemini agent output against a curated knowledge base of IRDAI regulations, "
    "MoRTH/NCRB statistics, and actuarial facts. A 🔴 **FLAGGED** result does **not** mean "
    "the model is actually hallucinating — it means the agent's narrative output "
    "(e.g. 'validated a portfolio of 500 policies...') uses different vocabulary than the "
    "short regulatory KB snippets, causing low TF-IDF overlap. "
    "**To improve RAG accuracy:** add your own source documents via the "
    "'📂 Add Custom RAG Documents' panel in the Agent Reasoning Chain tab — "
    "this lets RAG compare Gemini outputs against *your actual data sources* "
    "instead of only the built-in IRDAI/NCRB snippets. "
    "The **ValidationAgent is excluded from RAG** by design — its output is about "
    "data schema errors, not insurance domain facts."
)
...
if not engine.ready:
    st.markdown("""
Welcome to **TruckShield AI** — Full Agentic AI Architecture.

| Stage | Agent | Description |
|---|---|---|
| 1 | **Data Validation** |calls Gemini to write a data quality audit note; Validates all dataset columns; uses real claims data for XGBoost targets |
| 2 | **Climate Risk** |calls Gemini to identify the dominant peril; Climate zone modifiers + Route Risk Score (route type × highway × destinations) |
| 3 | **Risk Scoring** |calls Gemini to explain top risk drivers; Driver Behavior Score; Night Travel Risk score added |
| 4 | **Underwriting** |calls Gemini to justify the Accept/Refer/Decline mix; Claims history loading (count × severity) added to pricing |
| 5 | **Pricing** |calls Gemini to validate premiums vs market benchmarks; 6-field premium split: tp_pure/tp_gross (compulsory) + od_pure/od_gross (optional) + totals |

**Agent Interaction:**
| Cordinator | Description |
|---|---|
| **Orchestrator** |routes AgentMessages, logs inter-agent communication |
| **Reasoning Chain** |tab shows every agent's Gemini reasoning + message log |

👈 Add your Gemini API key in the sidebar, pick a data source, and click **Run 5-Agent Pipeline**.
Without a Gemini key, the pipeline runs in deterministic mode.
""")
    st.stop()
...
tab_names = ["💰 Get a Quote", "📊 Portfolio Analytics", "🌧️ Climate & Route Risk",
             "👤 Driver & Travel Risk", "✅ Underwriting", "📊 TP vs OD Analysis",
             "📥 Export Center", "🤖 Agent Reasoning Chain"]
...
# ════════════════════════════════════════════════════════════
# TAB 7 — AGENT REASONING CHAIN  (always visible after pipeline run)
# ════════════════════════════════════════════════════════════
with tabs[7]:
    st.subheader("🤖 Agent Reasoning Chain")
    st.caption(
        "This tab shows the **inter-agent communication log** — every AgentMessage "
        "passed between the 5 autonomous agents, dynamically spawned sub-agents, "
        "tool calls made by agents, retry attempts, and the Gemini reasoning each agent "
        "produced before executing its actuarial computation. "
        "This is what makes TruckShield a full **Agentic AI** system."
    )

    if not engine.orchestrator.reasoning_chain:
        if st.session_state.get("gemini_key"):
            st.info("Run the pipeline to see agent reasoning. Each agent will call Gemini "
                    "to justify its outputs before passing results to the next agent.")
        else:
            st.warning(
                "⚡ **Deterministic mode** — no Gemini API key provided.\n\n"
                "The 5-agent pipeline ran in fast deterministic mode. "
                "To unlock **full Agentic AI mode** with Gemini reasoning at each agent step, "
                "enter your Gemini API key in the sidebar and re-run the pipeline."
            )
    else:
        # ── Architecture diagram ──────────────────────────────────────
        st.markdown("### 🏗️ Full Agentic Architecture")
        st.markdown("""
```
┌─────────────────────────────────────────────────────────────────────────┐
│          ORCHESTRATOR  (tool registry + sub-agent registry)        │
│  Routes AgentMessages · Retry with backoff · Tools · Spawns sub-agents │
└──────┬──────────────────────────────────────────────────────────────────┘
       │  AgentMessage: validation_complete
       ▼
┌─────────────┐  reason()+retry  ┌─────────────────────────────────────┐
│  Agent 1    │ ───────────────► │ Tool: weather_tool (live conditions) │
│  Validation │ ◄─ Gemini text ─ │ "Is the data fit for model?"        │
└──────┬──────┘                  └─────────────────────────────────────┘
       │  AgentMessage: climate_scores
       ▼
┌─────────────┐  reason()+retry  ┌─────────────────────────────────────┐
│  Agent 2    │ ───────────────► │ Tool: weather_tool (state weather)  │
│  Climate    │ ◄─ Gemini text ─ │ "Which peril dominates?"            │
└──────┬──────┘                  └─────────────────────────────────────┘
       │  AgentMessage: risk_scores
       ▼
┌─────────────┐  reason()+retry  ┌─────────────────────────────────────┐
│  Agent 3    │ ───────────────► │ Tool: accident_stats_tool (NCRB)    │
│  Risk Score │  spawns ──────►  │ ► CargoSubAgent (Wikipedia tool)    │
│             │  sub-agents      │ ► DriverSubAgent (accident stats)   │
│             │ ◄─ Gemini text ─ │ "Top risk drivers?"                 │
└──────┬──────┘                  └─────────────────────────────────────┘
       │  AgentMessage: uw_decisions
       ▼
┌─────────────┐  reason()+retry  ┌─────────────────────────────────────┐
│  Agent 4    │ ───────────────► │ Tool: wikipedia_tool (IRDAI rules)  │
│  Underwrite │  spawns ──────►  │ ► FraudScreenSubAgent (if anomaly)  │
│             │ ◄─ Gemini text ─ │ "Is Accept/Refer mix right?"        │
└──────┬──────┘                  └─────────────────────────────────────┘
       │  AgentMessage: pricing_complete
       ▼
┌─────────────┐  reason()+retry  ┌─────────────────────────────────────┐
│  Agent 5    │ ───────────────► │ Tool: wikipedia_tool (XGBoost)      │
│  Pricing    │ ───────────────► │ Tool: accident_stats_tool (benchmark│
│             │ ◄─ Gemini text ─ │ "Is the pricing reasonable?"        │
└─────────────┘                  └─────────────────────────────────────┘
```
""")
        ...
        with col1:
            n_retries = sum(1 for s in engine.orchestrator.reasoning_chain
                            if "Retry" in s.get("step", "") or "retry" in s.get("step", ""))
            st.metric("🔄 Retry Attempts", n_retries, help="Times an agent looped and retried Gemini")
        with col2:
            n_subagents = len(engine.orchestrator.sub_agents)
            st.metric("🐣 Sub-agents Spawned", n_subagents, help="Dynamically created specialist agents")
        with col3:
            n_tools = sum(1 for s in engine.orchestrator.reasoning_chain
                          if "Tool" in s.get("step", "") or "[TOOL]" in s.get("step", ""))
            st.metric("🔧 Tool Calls", n_tools, help="External tools called by agents during reasoning")

        # ── Reasoning steps ──────────────────────────────────────────
        # ── Categorise steps ─────────────────────────────────────────
        MAIN_AGENTS = {
            "Agent 1 — Validation", "Agent 2 — Climate Risk",
            "Agent 3 — Risk Scoring", "Agent 4 — Underwriting", "Agent 5 — Pricing",
            "ValidationAgent", "ClimateAgent", "RiskScoringAgent",
            "UnderwritingAgent", "PricingAgent",
        }
        SUB_AGENTS = {"CargoSubAgent", "DriverSubAgent", "FraudScreenSubAgent"}

        def _is_tool_step(s):
            return "🔧" in s.get("step", "") or "[TOOL]" in s.get("step", "")
        def _is_retry_step(s):
            return ("🔄 Retry" in s.get("step", "") or "✅ Retry" in s.get("step", "")
                    or "⚠️ All" in s.get("step", ""))
        def _is_subagent_spawn(s):
            return "🐣 Sub-agent" in s.get("step", "")

        main_steps    = [s for s in engine.orchestrator.reasoning_chain
                         if s["agent"] in MAIN_AGENTS and not _is_tool_step(s) and not _is_retry_step(s)]
        subagent_steps = [s for s in engine.orchestrator.reasoning_chain
                          if s["agent"] in SUB_AGENTS or _is_subagent_spawn(s)]
        tool_steps    = [s for s in engine.orchestrator.reasoning_chain if _is_tool_step(s)]
        retry_steps   = [s for s in engine.orchestrator.reasoning_chain if _is_retry_step(s)]

        agent_colors = {
            "Agent 1 — Validation":   "#1565C0",
            "Agent 2 — Climate Risk": "#2E7D32",
            "Agent 3 — Risk Scoring": "#E65100",
            "Agent 4 — Underwriting": "#6A1B9A",
            "Agent 5 — Pricing":      "#00695C",
            "ValidationAgent":        "#1565C0",
            "ClimateAgent":           "#2E7D32",
            "RiskScoringAgent":       "#E65100",
            "UnderwritingAgent":      "#6A1B9A",
            "PricingAgent":           "#00695C",
            "CargoSubAgent":          "#BF360C",
            "DriverSubAgent":         "#4E342E",
            "FraudScreenSubAgent":    "#880E4F",
        }

        def _render_steps(steps, expanded_last=True):
            for i, step in enumerate(steps, 1):
                color = agent_colors.get(step["agent"], "#37474F")
                step_label = step["step"]
                with st.expander(
                    f"**{i}. {step['agent']}** — _{step_label}_ `[{step['time']}]`",
                    expanded=(expanded_last and i == len(steps))
                ):
                    st.markdown(
                        f"<div style='border-left:4px solid {color}; padding:10px 16px; "
                        f"background:#f8f9fa; border-radius:4px; color:#1a1a1a; "
                        f"font-size:0.95rem; line-height:1.6;'>"
                        f"{step['reasoning'].replace(chr(10), '<br>')}"
                        f"</div>",
                        unsafe_allow_html=True
                    )

        # ── Section 1: The 5 Main Agents ─────────────────────────────
        st.markdown("### 💬 5 Main Agent Reasoning Steps")
        st.caption("Gemini reasoning produced by each of the 5 core agents before executing its actuarial step.")
        if main_steps:
            _render_steps(main_steps, expanded_last=True)
        else:
            st.info("No main-agent reasoning steps logged yet.")

        # ── Section 2: Sub-agents (spawned dynamically) ──────────────
        st.markdown("---")
        st.markdown("### 🐣 Sub-Agent Activity (Dynamically Spawned)")
        st.caption(
            "Sub-agents are created at runtime by parent agents for specialised tasks. "
            "CargoSubAgent and DriverSubAgent are spawned by Agent 3. "
            "FraudScreenSubAgent is spawned by Agent 4 only when anomaly signals are detected."
        )
        if subagent_steps:
            _render_steps(subagent_steps, expanded_last=False)
        else:
            st.info("No sub-agents were spawned in this run (requires Gemini key + anomaly trigger for FraudScreen).")

        # ── FraudScreenSubAgent Impact Explainer ──────────────────────
        st.markdown("---")
        st.markdown("### 🚨 FraudScreenSubAgent — How It Affects Your Results")

        fraud_res = getattr(engine.orchestrator, "_fraud_screen_result", {})

        if not fraud_res:
            st.info(
                "**FraudScreenSubAgent was not triggered in this run.** "
                "It only activates in agentic mode (Gemini key required) when the portfolio "
                "shows anomaly signals: average claims ≥ 2.5 per policy OR average at-fault "
                "accidents ≥ 1.5 per policy. Your current portfolio is below these thresholds."
            )
            with st.expander("ℹ️ How does FraudScreenSubAgent work? (Click to learn)", expanded=False):
                st.markdown("""
**FraudScreenSubAgent** is a specialist sub-agent dynamically spawned by the Underwriting Agent
(Agent 4) when it detects unusual claim patterns across the portfolio.

**When is it triggered?**
- Portfolio average claims count ≥ 2.5 per policy, OR
- Portfolio average at-fault accidents ≥ 1.5 per policy

**What does it do?**

| Step | Action |
|------|--------|
| 1. Analyse | Reads portfolio-level claims, at-fault accidents, violation rates, vehicle age, and risk score |
| 2. Assess | Uses Gemini LLM to reason about whether the pattern is consistent with fraud or claim gaming |
| 3. Flag | Issues a **NONE / MODERATE / SEVERE** fraud flag based on the anomaly combination |
| 4. Load | Applies additional OD premium loading: 0% (NONE), 3% (MODERATE), or 7% (SEVERE) |
| 5. Upgrade decisions | SEVERE flag: upgrades individual ACCEPT→REFER (if ≥2 at-fault + ≥3 claims), REFER→DECLINE (if ≥3 at-fault + claim >₹3L) |

**Why does this make agentic mode give different premiums?**
Without the sub-agent (deterministic mode), only the base risk score and claims loading
affect the premium. With the sub-agent active, a **fraud loading of 3–7% is added on top**
of the existing OD loading, and some policies are escalated to a stricter underwriting bucket —
both of which directly increase the final gross premium for flagged policies.
""")
        else:
            flag_level    = fraud_res.get("flag_level", "NONE")
            fraud_loading = fraud_res.get("fraud_loading", 0)
            reasoning_txt = fraud_res.get("sub_reasoning", "Not available.")

            flag_color = {"NONE": "🟢", "MODERATE": "🟡", "SEVERE": "🔴"}.get(flag_level, "⚪")

            col1, col2, col3 = st.columns(3)
            col1.metric("Fraud Flag Level", f"{flag_color} {flag_level}")
            col2.metric("Additional OD Loading Applied", f"{fraud_loading*100:.0f}%",
                        help="This % was added to each non-DECLINE policy's OD premium denominator")
            if engine.processed_df is not None and "uw_fraud_loading" in engine.processed_df.columns:
                n_affected = (engine.processed_df["uw_fraud_loading"] > 0).sum()
                col3.metric("Policies Affected", f"{n_affected}")
            else:
                col3.metric("Policies Affected", "N/A")

            st.markdown("**How the result was reached:**")
            st.markdown(
                f"<div style='border-left:4px solid #880E4F; padding:10px 16px; "
                f"background:#fce4ec; border-radius:4px; color:#1a1a1a; "
                f"font-size:0.95rem; line-height:1.6;'>"
                f"{reasoning_txt.replace(chr(10), '<br>')}"
                f"</div>",
                unsafe_allow_html=True
            )

            st.markdown("**What changed in underwriting decisions?**")
            if flag_level == "SEVERE":
                st.warning(
                    "⚠️ **SEVERE flag active.** "
                    "Individual policies with ≥ 3 at-fault accidents AND claim amount > ₹3L were "
                    "escalated from REFER → DECLINE. "
                    "Policies with ≥ 2 at-fault accidents AND ≥ 3 claims were escalated from "
                    "ACCEPT → REFER. "
                    f"All non-DECLINE policies received a **{fraud_loading*100:.0f}% additional OD loading**, "
                    "which increases their gross OD premium."
                )
            elif flag_level == "MODERATE":
                st.warning(
                    "🟡 **MODERATE flag active.** "
                    "Severe individual outliers (≥ 3 at-fault + ≥ 4 claims) were escalated from "
                    "ACCEPT → REFER. "
                    f"All non-DECLINE policies received a **{fraud_loading*100:.0f}% additional OD loading**."
                )
            else:
                st.success("✅ No fraud flag raised — underwriting decisions and loading unchanged.")

            with st.expander("📘 Full explanation: how FraudScreenSubAgent contributes to premium", expanded=False):
                st.markdown(f"""
**FraudScreenSubAgent** was triggered because your portfolio exceeded the anomaly threshold.
Here is the exact contribution chain:

```
Portfolio data → UnderwritingAgent (Agent 4)
   │
   ├── Detects anomaly (mean_claims ≥ 2.5 OR mean_at_fault ≥ 1.5)
   │
   └── Spawns: FraudScreenSubAgent
         │
         ├── Calls {engine.orchestrator.get_llm_display_name()} with portfolio anomaly indicators
         ├── Receives: fraud flag = {flag_level}, fraud_loading = {fraud_loading*100:.0f}%
         └── Returns result to Orchestrator → stored as _fraud_screen_result
                   │
                   ▼
         _run_underwriting_core reads _fraud_screen_result
         ├── Applies uw_fraud_loading = {fraud_loading*100:.0f}% to each policy row
         └── Upgrades decision bucket for individual high-risk outliers
                   │
                   ▼
         _run_pricing_core reads uw_fraud_loading per policy
         └── total_loading = uw_extra_loading + uw_claims_loading + uw_fraud_loading
                   │
                   ▼
         OD gross premium = OD pure / (1 - expenses - profit - total_loading)
         → Higher total_loading → smaller denominator → HIGHER OD PREMIUM
```

**This is exactly why agentic mode (with Gemini) can produce a different premium
than deterministic mode** — the FraudScreenSubAgent adds an extra loading layer
that does not exist in the base formula pipeline.
""")

        # ── CargoSubAgent Impact Explainer ────────────────────────────
        st.markdown("---")
        st.markdown("### 🚛 CargoSubAgent — How It Affects Your Results")

        cargo_res = getattr(engine.orchestrator, "_cargo_sub_result", {})

        if not cargo_res:
            st.info(
                "**CargoSubAgent was not triggered in this run.** "
                "It only activates in agentic mode (Gemini key required) and is always "
                "spawned by Agent 3 (Risk Scoring) once per pipeline run."
            )
            with st.expander("ℹ️ How does CargoSubAgent work? (Click to learn)", expanded=False):
                st.markdown("""
**CargoSubAgent** is a specialist sub-agent dynamically spawned by Agent 3 (Risk Scoring Agent)
every time the pipeline runs in agentic mode.

**What does it do?**

| Step | Action |
|------|--------|
| 1. Identify | Reads the most common cargo category in your portfolio |
| 2. Research | Calls the **Wikipedia tool** to pull transport hazard data for that cargo type in India |
| 3. Analyse | Uses Gemini LLM to assess whether the cargo's risk score is appropriate and suggest mitigations |
| 4. Inform | Returns its analysis as a note that is injected into Agent 3's Gemini prompt |

**Does it change the numbers?**
CargoSubAgent does **not** directly change `cargo_risk_score` — that score is always set
deterministically from the cargo risk map (lookup table). What it does is enrich Agent 3's
Gemini reasoning with domain-specific cargo hazard context, which shapes the qualitative
analysis Agent 3 produces and the message it passes to Agent 4 (Underwriting).
""")
        else:
            cargo_name    = cargo_res.get("cargo", "N/A")
            cargo_score   = cargo_res.get("cargo_score", "N/A")
            cargo_reason  = cargo_res.get("sub_reasoning", "Not available.")

            col1, col2 = st.columns(2)
            col1.metric("Cargo Category Analysed", cargo_name)
            col2.metric("Cargo Risk Score (0–10)", cargo_score,
                        help="Set deterministically from the cargo risk map; CargoSubAgent validates this value")

            st.markdown("**Gemini analysis from CargoSubAgent:**")
            st.markdown(
                f"<div style='border-left:4px solid #BF360C; padding:10px 16px; "
                f"background:#fbe9e7; border-radius:4px; color:#1a1a1a; "
                f"font-size:0.95rem; line-height:1.6;'>"
                f"{cargo_reason.replace(chr(10), '<br>')}"
                f"</div>",
                unsafe_allow_html=True
            )

            with st.expander("📘 How CargoSubAgent contributes to the pipeline", expanded=False):
                st.markdown(f"""
**CargoSubAgent was spawned** because a Gemini API key was provided.
Here is the exact role it plays:

```
Portfolio data → RiskScoringAgent (Agent 3)
   │
   ├── Computes cargo_risk_score deterministically
   │     cargo_risk_score = cargo_risk_map["{cargo_name}"] = {cargo_score}/10
   │
   └── Spawns: CargoSubAgent
         │
         ├── Calls Wikipedia tool for "{cargo_name} transport risk India"
         ├── Calls {engine.orchestrator.get_llm_display_name()} with cargo score + wiki context
         └── Returns 2-3 sentence hazard analysis → injected into Agent 3 Gemini prompt
                   │
                   ▼
         Agent 3 Gemini prompt includes sub_cargo_note
         → Influences qualitative risk narrative sent to Agent 4
         → Agent 4 Gemini reasoning incorporates cargo hazard context
```

**Why does this NOT change the OD premium directly?**
`cargo_risk_score` feeds into `composite_risk_score` with a 20% weight:
```
composite = 0.20 × cargo_risk_score + 0.20 × exposure + 0.15 × (10 − safety) + ...
```
The score itself is deterministic. CargoSubAgent only adds **qualitative context** to
Agent 3's LLM narrative — it does not modify the formula. This is by design: the
actuary controls the score; the sub-agent explains and validates it.
""")

        # ── DriverSubAgent Impact Explainer ───────────────────────────
        st.markdown("---")
        st.markdown("### 👤 DriverSubAgent — How It Affects Your Results")

        driver_res = getattr(engine.orchestrator, "_driver_sub_result", {})

        if not driver_res:
            st.info(
                "**DriverSubAgent was not triggered in this run.** "
                "It only activates in agentic mode (Gemini key required) and is always "
                "spawned by Agent 3 (Risk Scoring) once per pipeline run."
            )
            with st.expander("ℹ️ How does DriverSubAgent work? (Click to learn)", expanded=False):
                st.markdown("""
**DriverSubAgent** is a specialist sub-agent dynamically spawned by Agent 3 (Risk Scoring Agent)
every time the pipeline runs in agentic mode, alongside CargoSubAgent.

**What does it do?**

| Step | Action |
|------|--------|
| 1. Identify | Reads portfolio-level driver behaviour statistics (experience, at-fault accidents, turnover, violations) |
| 2. Research | Calls the **NCRB accident_stats_tool** to pull state-level and route-level risk indices |
| 3. Analyse | Uses Gemini LLM to assess the driver profile against Indian fleet norms |
| 4. Inform | Returns its analysis as a note injected into Agent 3's Gemini prompt |

**Does it change the numbers?**
DriverSubAgent does **not** directly change `driver_behavior_score` — that score is always
computed deterministically using the weighted formula:
```
driver_behavior_score = 10 × (0.30 × exp_norm + 0.25 × training_sc
                               − 0.20 × fault_penalty − 0.15 × viol_penalty
                               − 0.10 × turnover_penalty)
```
What it does is enrich Agent 3's Gemini reasoning with real NCRB-grounded accident data,
shaping the qualitative driver risk analysis that flows into the Agent 4 Underwriting decision.
""")
        else:
            driver_beh   = driver_res.get("driver_beh", "N/A")
            avg_exp      = driver_res.get("avg_exp", "N/A")
            at_fault     = driver_res.get("at_fault", "N/A")
            turnover     = driver_res.get("turnover", "N/A")
            driver_reason = driver_res.get("sub_reasoning", "Not available.")

            col1, col2, col3, col4 = st.columns(4)
            col1.metric("Driver Behaviour Score", f"{driver_beh}/10",
                        help="Higher = safer; computed from experience, training, violations, at-fault accidents")
            col2.metric("Avg Driver Experience", f"{avg_exp} yrs")
            col3.metric("At-Fault Accidents (3yr)", at_fault)
            col4.metric("Driver Turnover Rate", f"{turnover}%/yr")

            st.markdown("**Gemini analysis from DriverSubAgent:**")
            st.markdown(
                f"<div style='border-left:4px solid #4E342E; padding:10px 16px; "
                f"background:#efebe9; border-radius:4px; color:#1a1a1a; "
                f"font-size:0.95rem; line-height:1.6;'>"
                f"{driver_reason.replace(chr(10), '<br>')}"
                f"</div>",
                unsafe_allow_html=True
            )

            with st.expander("📘 How DriverSubAgent contributes to the pipeline", expanded=False):
                st.markdown(f"""
**DriverSubAgent was spawned** because a Gemini API key was provided.
Here is the exact role it plays:

```
Portfolio data → RiskScoringAgent (Agent 3)
   │
   ├── Computes driver_behavior_score deterministically
   │     = 10 × (0.30×exp + 0.25×training − 0.20×fault − 0.15×viol − 0.10×turnover)
   │     Portfolio mean: {driver_beh}/10
   │
   └── Spawns: DriverSubAgent
         │
         ├── Calls accident_stats_tool (NCRB data) for state + route context
         ├── Calls {engine.orchestrator.get_llm_display_name()} with driver stats + NCRB data
         └── Returns 2-3 sentence driver risk assessment → injected into Agent 3 Gemini prompt
                   │
                   ▼
         Agent 3 Gemini prompt includes sub_driver_note
         → Influences qualitative risk narrative sent to Agent 4
         → Agent 4 Gemini reasoning considers driver behaviour context
```

**Why does this NOT change the OD premium directly?**
`driver_behavior_score` feeds into `composite_risk_score` with a 15% inverted weight:
```
composite = ... + 0.15 × (10 − driver_behavior_score) + ...
```
A lower (worse) driver score increases the composite, which can push a policy from
ACCEPT to REFER and increase `uw_extra_loading`. But this is driven by the deterministic
formula — DriverSubAgent only adds **qualitative NCRB-grounded context** to Agent 3's
LLM narrative; it does not modify any formula parameter.
""")

        # ── RAG HALLUCINATION GUARD SECTION ─────────────────────────
        st.markdown("---")
        st.markdown("### 🔍 RAG — Hallucination Guard")
        st.caption(
            "Every Gemini response is automatically verified against a curated knowledge base "
            "of IRDAI regulations, NCRB/MoRTH statistics, and actuarial facts. "
            "The RAG guard uses TF-IDF cosine similarity to detect when the LLM output "
            "diverges from verified domain knowledge."
        )

        with st.expander("ℹ️ How the RAG Hallucination Guard works (click to expand)", expanded=False):
            st.markdown("""
**What is RAG?**
Retrieval-Augmented Generation (RAG) is a technique that grounds LLM outputs by retrieving
relevant verified facts *before* the model responds, and then checking the response against
those facts. Here, RAG acts as a **post-generation verifier** — it checks every Gemini
output after it is produced.

**Why is RAG needed here?**
Gemini (and Wikipedia) can hallucinate:
- Wrong IRDAI tariff figures (e.g., wrong TP premium amounts)
- Wrong NCB percentages or eligibility rules
- Wrong MoRTH/NCRB accident statistics
- Wrong cargo risk classifications or state risk rankings

**Three-layer knowledge base:**
| Layer | Source | Content |
|-------|--------|---------|
| 1 — Embedded KB | Hand-curated | IRDAI rules, NCRB data, actuarial methods |
| 2 — Wikipedia Live | REST API (open-source) | Technical background on IRDAI, XGBoost, BS norms, etc. |
| 3 — Custom User Docs | Uploaded by you | Your own regulatory reports, actuarial papers, data docs |

**Similarity thresholds (adjusted for domain vocabulary):**
| Score | Status | Meaning |
|-------|--------|---------|
| ≥ 0.12 | 🟢 VERIFIED | Output aligns with KB |
| 0.05 – 0.12 | 🟡 CAUTION | Low overlap; review key figures |
| < 0.05 | 🔴 FLAGGED | Very low overlap; possible hallucination |

**Why do I see FLAGGED results even when the model seems correct?**
This is a known limitation of TF-IDF similarity. The agent outputs are long narrative
paragraphs ("I have validated a portfolio of 500 policies…") while the KB entries are
short, specialised regulatory snippets ("Third Party TP motor insurance is compulsory…").
The vocabulary overlap is naturally low even when the content is factually correct.

**How to improve RAG accuracy:**
Add your own source documents via the **📂 Add Custom RAG Documents** panel below.
The more domain-specific text you add, the better the RAG can verify agent outputs.

**Which agents are RAG-checked?**
- ✅ **ClimateAgent, RiskScoringAgent, UnderwritingAgent, PricingAgent** — checked against the KB
- ⬜ **ValidationAgent** — excluded: output is about data schema errors, not domain facts
- 🔴 **CargoSubAgent, DriverSubAgent, FraudScreenSubAgent** — **now RAG-checked**. These sub-agents pull context from Wikipedia via the `wikipedia_tool`. Without user-uploaded documentation they will show **FLAGGED/CAUTION** because Wikipedia vocabulary has low overlap with the verified IRDAI/NCRB KB — this correctly signals that their reasoning is grounded in Wikipedia, not verified sources. Upload your model documentation via **📂 Add Custom RAG Documents** below to resolve this.

Thresholds are conservative because insurance regulatory text uses specialised vocabulary
that naturally has low overlap with general prose.
""")

        # ── RAG KB status ─────────────────────────────────────────────
        st.markdown("#### 📚 RAG Knowledge Base Status")
        kb_col1, kb_col2, kb_col3 = st.columns(3)
        kb_col1.metric("Embedded KB Documents", len(RAG_EMBEDDED_KB),
                       help="Verified IRDAI/NCRB/MoRTH facts pre-loaded at startup")
        total_kb = len(RAG_KB._docs)
        wiki_added = total_kb - len(RAG_EMBEDDED_KB)
        kb_col2.metric("Wikipedia Articles Added", wiki_added,
                       help="Open-source Wikipedia articles fetched at session start")
        kb_col3.metric("Total KB Size", total_kb,
                       help="Total documents available for hallucination checking")

        with st.expander(
            f"📋 Wikipedia fetch log ({len(RAG_KB.fetch_log)} entries)",
            expanded=(len(RAG_KB.fetch_log) > 0 and not any(l.startswith("✅") for l in RAG_KB.fetch_log))
        ):
            if RAG_KB.fetch_log:
                for line in RAG_KB.fetch_log:
                    st.text(line)
            else:
                st.info(
                    "Wikipedia bootstrap has not run yet or produced no log entries. "
                    "If Wikipedia Articles Added shows 0, it means the Wikipedia API "
                    "was unreachable from this environment. RAG still works using the "
                    f"{len(RAG_EMBEDDED_KB)} embedded IRDAI/NCRB documents."
                )

        # ── Custom RAG document input ──────────────────────────────────
        st.markdown("---")
        st.markdown("#### 📂 Add Custom RAG Documents")
        st.caption(
            "Add your own source documents (regulatory reports, actuarial papers, "
            "data documentation) so RAG checks Gemini outputs against *your actual sources* "
            "instead of only the built-in KB. This is the recommended fix when you see "
            "100% FLAGGED results — it means the built-in KB has low vocabulary overlap "
            "with the agent outputs, not that the model is actually hallucinating."
        )

        rag_input_method = st.radio(
            "How do you want to add documents?",
            ["Paste text directly", "Google Drive link (public/shared)", "Upload a .txt file"],
            horizontal=True,
            key="rag_input_method",
        )

        if rag_input_method == "Paste text directly":
            pasted_text = st.text_area(
                "Paste document text here (e.g. copy from a regulatory PDF or report):",
                height=150, key="rag_paste_text", placeholder="Paste any relevant text here…"
            )
            paste_label = st.text_input("Document label (optional):", value="Pasted document", key="rag_paste_label")
            if st.button("➕ Add pasted text to RAG KB", key="rag_add_paste"):
                if pasted_text and len(pasted_text.strip()) > 30:
                    RAG_KB.add_user_document(pasted_text.strip(), source_label=paste_label or "Pasted document")
                    st.success(f"✅ Added {len(pasted_text)} characters to RAG KB. Re-run the pipeline to see improved RAG scores.")
                else:
                    st.warning("Please paste at least 30 characters of text.")

        elif rag_input_method == "Google Drive link (public/shared)":
            drive_url = st.text_input(
                "Google Drive / Docs link (must be publicly shared or 'Anyone with link can view'):",
                key="rag_drive_url",
                placeholder="https://docs.google.com/document/d/..."
            )
            if st.button("➕ Fetch and add to RAG KB", key="rag_add_drive"):
                if drive_url and drive_url.startswith("http"):
                    with st.spinner("Fetching document…"):
                        success = RAG_KB.fetch_google_drive_text(drive_url)
                    if success:
                        st.success("✅ Document fetched and added to RAG KB. Re-run the pipeline to see improved RAG scores.")
                    else:
                        st.error("Could not fetch the document. Make sure sharing is set to 'Anyone with the link can view'. Check the fetch log below.")
                else:
                    st.warning("Please enter a valid URL starting with https://")

        elif rag_input_method == "Upload a .txt file":
            uploaded_txt = st.file_uploader("Upload a .txt document:", type=["txt"], key="rag_txt_upload")
            txt_label = st.text_input("Document label (optional):", value="Uploaded document", key="rag_txt_label")
            if uploaded_txt is not None:
                if st.button("➕ Add uploaded file to RAG KB", key="rag_add_txt"):
                    try:
                        txt_content = uploaded_txt.read().decode("utf-8", errors="replace")
                        RAG_KB.add_user_document(txt_content.strip(), source_label=txt_label or uploaded_txt.name)
                        st.success(f"✅ Added '{uploaded_txt.name}' ({len(txt_content)} chars) to RAG KB. Re-run the pipeline to see improved RAG scores.")
                    except Exception as e:
                        st.error(f"Could not read file: {e}")

        _user_docs_added = getattr(RAG_KB, "_user_docs_added", 0)
        if _user_docs_added > 0:
            st.info(f"📎 {_user_docs_added} custom document(s) currently in RAG KB "
                    f"(in addition to {len(RAG_EMBEDDED_KB)} embedded documents). "
                    f"Total KB size: {len(RAG_KB._docs)} documents.")

        st.markdown("---")
        rag_results = getattr(engine.orchestrator, "_rag_results", [])
        if not rag_results:
            _key_present = bool(st.session_state.get("gemini_key", ""))
            if not engine.orchestrator.reasoning_chain:
                # Pipeline hasn't run yet
                if _key_present:
                    st.info(
                        "✅ Gemini API key is set. **Run the pipeline** to see RAG verification results. "
                        "All 7 agents (4 main + 3 sub-agents) will be checked."
                    )
                else:
                    st.warning(
                        "🔑 **Enter your Gemini API key** in Section 3 of the sidebar (above the Run button), "
                        "then run the pipeline. Without a key, the pipeline runs in deterministic mode "
                        "and RAG cannot verify Gemini outputs (there are none)."
                    )
            else:
                # Pipeline ran but produced no RAG results
                if _key_present:
                    st.warning(
                        "⚠️ **Pipeline ran with an API key but RAG produced no results.** "
                        "Check the '⚠️ RAG Check Error' entries in the Reasoning Chain tab above "
                        "for the exact error. Common causes: key invalid/quota exceeded, "
                        "or Gemini returned responses shorter than 20 characters (retry threshold). "
                        "Re-enter your key and re-run the pipeline."
                    )
                else:
                    st.info(
                        "⚡ Pipeline ran in **deterministic mode** (no Gemini key). "
                        "Enter your Gemini API key in Section 3 of the sidebar and re-run "
                        "the pipeline to activate AI reasoning and RAG verification."
                    )
        else:
            # ── Summary metrics ─────────────────────────────────────
            st.markdown("#### 🧾 Verification Summary — This Pipeline Run")
            # -- "no user docs" warning banner --
            if not RAG_KB.has_user_docs():
                st.warning(
                    "📤 **No user documentation uploaded.** "
                    "Sub-agents (CargoSubAgent, DriverSubAgent, FraudScreenSubAgent) are "
                    "currently sourcing context from Wikipedia via the wikipedia_tool. "
                    "Their RAG results will show FLAGGED/CAUTION because Wikipedia has low "
                    "vocabulary overlap with the verified IRDAI/NCRB KB. "
                    "Upload your actual model documentation via '📂 Add Custom RAG Documents' "
                    "below to anchor their reasoning against verified sources."
                )
            else:
                st.success(
                    f"✅ **{RAG_KB._user_docs_added} user document(s) active in RAG KB.** "
                    "Sub-agents will now prefer your uploaded documentation over Wikipedia. "
                    "Re-run the pipeline to see improved RAG grounding."
                )
            st.caption(
                "⬜ **Excluded from RAG** by design: ValidationAgent only (data quality output). "
                "All 4 main agents + 3 sub-agents (CargoSubAgent, DriverSubAgent, FraudScreenSubAgent) "
                "are RAG-checked. Sub-agents are flagged when sourcing from Wikipedia — "
                "upload documentation to resolve. Counts below reflect all 7 checked agents."
            )
            n_verified = sum(1 for r in rag_results if r["status"] == "VERIFIED")
            n_caution  = sum(1 for r in rag_results if r["status"] == "CAUTION")
            n_flagged  = sum(1 for r in rag_results if r["status"] == "FLAGGED")

            rc1, rc2, rc3, rc4 = st.columns(4)
            rc1.metric("Total Checks", len(rag_results))
            rc2.metric("🟢 VERIFIED",  n_verified,
                       delta=f"{n_verified/max(len(rag_results),1)*100:.0f}%",
                       delta_color="normal")
            rc3.metric("🟡 CAUTION",   n_caution,
                       delta=f"{n_caution/max(len(rag_results),1)*100:.0f}%" if n_caution else None,
                       delta_color="inverse" if n_caution else "off")
            rc4.metric("🔴 FLAGGED",   n_flagged,
                       delta=f"{n_flagged/max(len(rag_results),1)*100:.0f}%" if n_flagged else None,
                       delta_color="inverse" if n_flagged else "off")

            # ── Individual result cards ──────────────────────────────
            st.markdown("#### 🔬 Detailed Verification Results")
            st.caption(
                "Each card shows: the agent that produced the output, the RAG status, "
                "the similarity score, the retrieved evidence used for comparison, "
                "and a snippet of the Gemini output that was checked."
            )

            STATUS_COLORS = {
                "VERIFIED": ("#e8f5e9", "#2e7d32", "🟢"),
                "CAUTION":  ("#fff8e1", "#f57f17", "🟡"),
                "FLAGGED":  ("#ffebee", "#c62828", "🔴"),
            }

            for i, r in enumerate(rag_results, 1):
                bg, border, icon = STATUS_COLORS.get(r["status"], ("#f5f5f5", "#9e9e9e", "⚪"))
                with st.expander(
                    f"**{i}. {r['agent']}** — {icon} {r['status']} "
                    f"(similarity={r['similarity']:.3f})",
                    expanded=(r["status"] == "FLAGGED")
                ):
                    # Data source badge colour
                    ds = r.get("data_source", "embedded_kb")
                    ds_colour = {"user_docs": "#1B5E20", "wikipedia": "#B71C1C", "embedded_kb": "#0D47A1"}.get(ds, "#555")
                    ds_label  = r.get("source_label", ds)
                    subagent_note = (" — Wikipedia-sourced (no user docs uploaded)" if r.get("is_subagent") and ds == "wikipedia" else "")
                    st.markdown(
                        f"<div style='border-left:4px solid {border}; padding:8px 14px; "
                        f"background:{bg}; border-radius:4px; margin-bottom:8px;'>"
                        f"<b>Status:</b> {icon} {r['status']}<br>"
                        f"<b>Data Source:</b> <span style='color:{ds_colour};font-weight:bold;'>"
                        f"{ds_label}{subagent_note}</span><br>"
                        f"<b>Similarity Score:</b> {r['similarity']:.4f} "
                        f"(threshold: VERIFIED ≥ {RAG_MEDIUM_THRESHOLD}, "
                        f"FLAGGED < {RAG_SIMILARITY_THRESHOLD})<br>"
                        f"<b>Assessment:</b> {r['flag_reason']}"
                        f"</div>",
                        unsafe_allow_html=True
                    )

                    if r.get("evidence"):
                        st.markdown("**📖 Retrieved KB evidence used for comparison:**")
                        for ev in r["evidence"]:
                            st.info(f"🗂️ {ev}")

                    if r.get("gemini_snippet"):
                        st.markdown("**🤖 Gemini output snippet checked:**")
                        st.text(r["gemini_snippet"][:400] + ("…" if len(r["gemini_snippet"]) > 400 else ""))

            # ── RAG knowledge base search tool ───────────────────────
            st.markdown("---")
            st.markdown("#### 🔎 Search the RAG Knowledge Base")
            st.caption(
                "Query the verified knowledge base directly to see what facts are available "
                "for any topic. This helps you understand what the RAG guard is checking against."
            )
            rag_query = st.text_input(
                "Search query (e.g. 'NCB discount', 'flood risk Assam', 'XGBoost frequency model')",
                key="rag_search_query"
            )
            if rag_query:
                kb_results = RAG_KB.retrieve(rag_query, top_k=3)
                if kb_results:
                    for doc in kb_results:
                        with st.expander(f"📄 [{doc['source']}] {doc['topic']}", expanded=True):
                            st.write(doc["text"])
                else:
                    st.info("No documents found for that query.")

        # ── Section 3: Tool calls ─────────────────────────────────────
        st.markdown("---")
        st.markdown("### 🔧 Tool Calls by Agents")
        st.caption("External tool invocations made by agents during their reasoning pipeline (weather, Wikipedia, NCRB stats).")
        if tool_steps:
            _render_steps(tool_steps, expanded_last=False)
        else:
            st.info("No tool calls logged (tools run silently in deterministic mode).")

        # ── Section 4: Retry log ──────────────────────────────────────
        if retry_steps:
            st.markdown("---")
            st.markdown("### 🔄 Retry Log")
            st.caption("Gemini call retries with exponential backoff (up to 3 attempts per agent).")
            _render_steps(retry_steps, expanded_last=False)

        st.markdown("---")
        # ── Message log ──────────────────────────────────────────────
        st.markdown("### 📨 Inter-Agent Message Log")
        st.caption("Every AgentMessage passed between agents via the Orchestrator.")
        if engine.orchestrator.message_log:
            msg_rows = []
            for m in engine.orchestrator.message_log:
                # Format payload as readable key: value lines
                payload = m.payload
                ...
                msg_rows.append({
                    "Time (IST)": m.timestamp,
                    "From":       m.sender.replace("Agent", "").strip(),
                    "To":         m.receiver.replace("Agent", "").strip(),
                    "Type":       m.msg_type,
                    "Payload":    payload_str,
                })
            st.dataframe(
                ...
                hide_index=True,
                column_config={
                    "Time (IST)": st.column_config.TextColumn(width="small"),
        ...
        st.markdown("### 🧠 Agent Memory Summaries")
        mem_cols = st.columns(5)
        agents_list = [
            engine.agent_validation, engine.agent_climate,
            engine.agent_risk, engine.agent_underwriting, engine.agent_pricing,
        ]
        for col, agent in zip(mem_cols, agents_list):
            with col:
                label = agent.name.replace("Agent", "").strip()
                st.markdown(f"**{label}**")
                st.caption(f"📥 {len(agent.memory.inbox)} received")
                st.caption(f"📤 {len(agent.memory.outbox)} sent")
                if agent.memory.outbox:
                    last = agent.memory.outbox[-1]
                    st.caption(f"Last: `{last.msg_type}`")

# ════════════════════════════════════════════════════════════
# TAB 8 — AI ASSISTANT  (optional) — TAB 7
# ════════════════════════════════════════════════════════════
if st.session_state.get("gemini_key"):
    with tabs[8]:
        st.subheader("💬 AI Assistant")
        st.caption("Powered by Gemini. For exact premium quotes, use the **Get a Quote** tab.")
