...
# ValidationAgent output is about schema errors / missing values — there is no domain KB
# entry that covers "policy row 42 has a missing GVW field", so comparing it against
# IRDAI tariff documents always scores near-zero and produces misleading FLAGGED results.
RAG_EXCLUDED_AGENTS = {"ValidationAgent"}
...
class RAGKnowledgeBase:
    ...
    def retrieve(self, query: str, top_k: int = 3) -> list[dict]:
        ...
        scored = [
            (self._cosine(q_vec, q_norm, entry), entry["doc"])
            for entry in self._tfidf_index
        ]
    ...
    def check_hallucination(self, gemini_text: str,
        ...
        if similarity < RAG_SIMILARITY_THRESHOLD:
            status      = "FLAGGED"
            flag_reason = (
                f"Similarity score {similarity:.3f} is below the high-risk threshold "
                f"({RAG_SIMILARITY_THRESHOLD}). The LLM output has very low overlap with "
                f"verified domain knowledge. Review manually."
            )
        elif similarity < RAG_MEDIUM_THRESHOLD:
            status      = "CAUTION"
            flag_reason = (
                f"Similarity score {similarity:.3f} is below the medium-risk threshold "
                f"({RAG_MEDIUM_THRESHOLD}). The LLM output may contain imprecise wording. "
                f"Cross-check key figures."
            )
        else:
            status      = "VERIFIED"
            flag_reason = (
                f"Similarity score {similarity:.3f} ≥ {RAG_MEDIUM_THRESHOLD}. "
                f"Output aligns with retrieved knowledge base context."
            )
            ...
            try:
                ...
                if agent_name not in RAG_EXCLUDED_AGENTS:
                  try:
                    ...
                    pass   # RAG check must never block the pipeline
                else:
                    # Log that this agent was excluded from RAG checking
                    self.log_reasoning(
                        agent_name,
                        "🔍 RAG Hallucination Check",
                        "⬜ RAG check skipped for ValidationAgent — its output describes "
                        "data quality issues (missing fields, type errors) which have no "
                        "matching domain KB entries. Comparing against IRDAI/NCRB documents "
                        "would always score near-zero and give misleading FLAGGED results."
                    )
                # ── END RAG GUARD ──────────────────────────────────────
...
with st.sidebar:
    ...
    if gemini_key:
        ...
        engine.set_gemini_key(gemini_key)   # wire key into Orchestrator for agent reasoning
...
- ✅ ClimateAgent, RiskScoringAgent, UnderwritingAgent, PricingAgent — checked
- ⬜ ValidationAgent — **excluded by design**: its output describes data schema errors
  (missing fields, wrong types) which have no matching domain KB entries. Comparing
  these outputs against IRDAI tariff documents always scores near-zero and gives
  misleading FLAGGED results.
...
""")
        ...
        if RAG_KB.fetch_log:
            with st.expander("📋 Wikipedia fetch log", expanded=False):
        ...
        if RAG_KB._user_docs_added > 0:
            st.info(f"📎 {RAG_KB._user_docs_added} custom document(s) currently in RAG KB "
        ...
        if not rag_results:
            if not engine.orchestrator.reasoning_chain:
                st.info(
                    "Run the pipeline with a **Gemini API key** to see RAG verification results. "
                    "Every agent's Gemini output will be checked against the knowledge base."
                )
            else:
                st.info(
                    "RAG verification is active but no Gemini calls were made in this run "
                    "(deterministic mode). Add a Gemini API key to see live verification."
                )
        else:
            # ── Summary metrics ─────────────────────────────────────
            st.markdown("#### 🧾 Verification Summary — This Pipeline Run")
            st.caption(
                "⬜ **ValidationAgent is excluded from RAG** by design — "
                "its output is about data schema errors, not insurance domain facts. "
                "The counts below reflect only the 4 domain agents (Climate, Risk, Underwriting, Pricing)."
            )
